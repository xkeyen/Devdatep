...
$old_error_handler = set_error_handler("item_change_status_mob_scriptcase_error_handler");

function item_change_status_mob_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="", $err_usr=false)
{
    ...
    if (isset($_SESSION['scriptcase']['item_change_status_mob']['contr_erro']) && $_SESSION['scriptcase']['item_change_status_mob']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_item_change_status_mob->contr_item_change_status_mob->Apl_com_erro = true;
            if ($inicial_item_change_status_mob->contr_item_change_status_mob->NM_ajax_flag)
            {
                $inicial_item_change_status_mob->contr_item_change_status_mob->NM_ajax_info['errList']['geral_item_change_status_mob'][] = $msg;
                return;
            }
            item_change_status_mob_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function item_change_status_mob_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['item_change_status_mob'])  && '' != $_SESSION['scriptcase']['error_icon']['item_change_status_mob'])  ? $_SESSION['scriptcase']['error_icon']['item_change_status_mob']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['item_change_status_mob']) && '' != $_SESSION['scriptcase']['error_close']['item_change_status_mob']) ? $_SESSION['scriptcase']['error_close']['item_change_status_mob'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['item_change_status_mob']) && '' != $_SESSION['scriptcase']['error_span_title']['item_change_status_mob']) ? $_SESSION['scriptcase']['error_span_title']['item_change_status_mob'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['item_change_status_mob']) && '' != $_SESSION['scriptcase']['error_icon_title']['item_change_status_mob']) ? $_SESSION['scriptcase']['error_icon_title']['item_change_status_mob'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['item_change_status']['error_buffer'] .= ob_get_flush();
}
