<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="id_order" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['id_order']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_tb_orders']['parms'])) {echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_tb_orders']['parms']);} ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_tb_orders"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?> 
var var_btn_merge_id_order = "<?php echo $this->form_encode_input($this->nmgp_dados_form['id_order']); ?>";
function sc_btn_merge()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_merge(); }, 500);
      return;
    }
    sc_btn_merge_ok();
}
function sc_btn_merge_cancel()
{
}
function sc_btn_merge_ok()
{
  nm_gp_submit('<?php echo $this->Ini->link_form_merge_order_table_edit; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "id_order*scin' + var_btn_merge_id_order + '*scoutNM_btn_insert*scinN*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scout"; ?>', '', '_self', '0', '0', 'form_merge_order_table');
}
var Crtl_btn_close_order = false;
function sc_btn_close_order()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_close_order(); }, 500);
      return;
    }
    if (Crtl_btn_close_order) {return;}
    sc_btn_close_order_ok();
}
function sc_btn_close_order_cancel()
{
}
function sc_btn_close_order_ok()
{
    Crtl_btn_close_order = true;
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?close_order?@?";
    document.F1.action = "./";
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_tb_orders']['where_detal']))
  {
?>  
...
function sc_inline_form(seqRow, keyParams, width, height)
{
  ...
  nm_gp_submit('<?php echo $this->Ini->link_form_tb_orders_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinN*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutsc_seq_row*scin' + seqRow + '*scout' + callParams, '', 'modal', height, width);
}

function sc_inline_form_add(width, height)
{
  nm_gp_submit('<?php echo $this->Ini->link_form_tb_orders_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinY*scoutNM_btn_update*scinN*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutnmgp_opcao*scinnovo*scout', '', 'modal', height, width);
}
...
{
   nm_field_disabled("order_status=disabled;dt_order_start=disabled", "");
