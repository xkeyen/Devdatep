...
function scEventControl_init(iSeqRow) {
  scEventControl_data["id_kitchen_status_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
  scEventControl_data["kitchen_status_descr_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
}

function scEventControl_active(iSeqRow) {
  if (scEventControl_data["id_kitchen_status_" + iSeqRow] && scEventControl_data["id_kitchen_status_" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["id_kitchen_status_" + iSeqRow] && scEventControl_data["id_kitchen_status_" + iSeqRow]["change"]) {
    return true;
  }
  if (scEventControl_data["kitchen_status_descr_" + iSeqRow] && scEventControl_data["kitchen_status_descr_" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["kitchen_status_descr_" + iSeqRow] && scEventControl_data["kitchen_status_descr_" + iSeqRow]["change"]) {
    return true;
  }
  return false;
...
function scJQEventsAdd(iSeqRow) {
  $('#id_sc_field_id_kitchen_status_' + iSeqRow).bind('blur', function() { sc_form_tb_kitchen_status_id_kitchen_status__onblur(this, iSeqRow) })
                                                .bind('change', function() { sc_form_tb_kitchen_status_id_kitchen_status__onchange(this, iSeqRow) })
                                                .bind('focus', function() { sc_form_tb_kitchen_status_id_kitchen_status__onfocus(this, iSeqRow) });
  $('#id_sc_field_kitchen_status_descr_' + iSeqRow).bind('blur', function() { sc_form_tb_kitchen_status_kitchen_status_descr__onblur(this, iSeqRow) })
                                                   .bind('change', function() { sc_form_tb_kitchen_status_kitchen_status_descr__onchange(this, iSeqRow) })
                                                   .bind('focus', function() { sc_form_tb_kitchen_status_kitchen_status_descr__onfocus(this, iSeqRow) });
} // scJQEventsAdd

function sc_form_tb_kitchen_status_id_kitchen_status__onblur(oThis, iSeqRow) {
  do_ajax_form_tb_kitchen_status_validate_id_kitchen_status_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_tb_kitchen_status_id_kitchen_status__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_tb_kitchen_status_id_kitchen_status__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}

function sc_form_tb_kitchen_status_kitchen_status_descr__onblur(oThis, iSeqRow) {
  do_ajax_form_tb_kitchen_status_validate_kitchen_status_descr_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_tb_kitchen_status_kitchen_status_descr__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_tb_kitchen_status_kitchen_status_descr__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}
...
function displayChange_block_0(status) {
	displayChange_field("id_kitchen_status_", "", status);
	displayChange_field("kitchen_status_descr_", "", status);
}

function displayChange_row(row, status) {
	displayChange_field_id_kitchen_status_(row, status);
	displayChange_field_kitchen_status_descr_(row, status);
}

function displayChange_field(field, row, status) {
	if ("id_kitchen_status_" == field) {
		displayChange_field_id_kitchen_status_(row, status);
	}
	if ("kitchen_status_descr_" == field) {
		displayChange_field_kitchen_status_descr_(row, status);
	}
}

function displayChange_field_id_kitchen_status_(row, status) {
    var fieldId;
}

function displayChange_field_kitchen_status_descr_(row, status) {
    var fieldId;
}
...
function scHidePage(pageNo) {
	$("#id_form_tb_kitchen_status_form" + pageNo).hide();
}
            ...
            if(!$(t).is('img')){
                ...
                switch(field){
                     ...
                     if(usa_read_only && $('a',$('#id_read_on_'+field+iSeqRow)).length == 0){
                         $(t).html("<a href=\"javascript:nm_mostra_doc('0', '"+rs2+"', 'form_tb_kitchen_status')\">"+$('#id_read_on_'+field+iSeqRow).text()+"</a>");
                     }
                }
