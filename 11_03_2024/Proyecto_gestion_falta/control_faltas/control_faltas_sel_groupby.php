<?php
   ...
   if (!isset($_SESSION['sc_session']))
   {
       ...
       if (is_file($root . $_SESSION['scriptcase']['control_faltas']['glo_nm_path_imag_temp'] . "/sc_apl_default_Proyecto_gestion_falta.txt"))
       {
?>
...
<?php
   ...
   $Sel_Groupby = new control_faltas_sel_Groupby(); 
   $Sel_Groupby->Sel_Groupby_init();
   
class control_faltas_sel_Groupby
{
function Sel_Groupby_init()
{
   ...
   if ($this->restore) {
       $groupby_atual = $_SESSION['sc_session'][$sc_init]['control_faltas']['dados_orig_gb']['all']['SC_Ind_Groupby'];
   }
   else {
       $groupby_atual = $_SESSION['sc_session'][$sc_init]['control_faltas']['SC_Ind_Groupby'];
   }
...
function Sel_processa_out($sel_groupby)
{
   ...
   {
       if (isset($_SESSION['sc_session'][$sc_init]['control_faltas']['using_summary_cache'])) {
           unset($_SESSION['sc_session'][$sc_init]['control_faltas']['using_summary_cache']);
       }
       $_SESSION['sc_session'][$sc_init]['control_faltas']['SC_Ind_Groupby']     = $sel_groupby;
       $_SESSION['sc_session'][$sc_init]['control_faltas']['contr_array_resumo'] = "NAO";
       $_SESSION['sc_session'][$sc_init]['control_faltas']['contr_total_geral']  = "NAO";
       unset($_SESSION['sc_session'][$sc_init]['control_faltas']['ordem_quebra']);
       unset($_SESSION['sc_session'][$sc_init]['control_faltas']['ordem_select']);
       unset($_SESSION['sc_session'][$sc_init]['control_faltas']['tot_geral']);
       unset($_SESSION['sc_session'][$sc_init]['control_faltas']['Page_break_PDF']);
       unset($_SESSION['sc_session'][$sc_init]['control_faltas']['arr_total']);
       unset($_SESSION['sc_session'][$sc_init]['control_faltas']['pivot_group_by']);
       unset($_SESSION['sc_session'][$sc_init]['control_faltas']['pivot_x_axys']);
       unset($_SESSION['sc_session'][$sc_init]['control_faltas']['pivot_y_axys']);
       unset($_SESSION['sc_session'][$sc_init]['control_faltas']['pivot_fill']);
       unset($_SESSION['sc_session'][$sc_init]['control_faltas']['pivot_order']);
       unset($_SESSION['sc_session'][$sc_init]['control_faltas']['pivot_order_col']);
       unset($_SESSION['sc_session'][$sc_init]['control_faltas']['pivot_order_level']);
       unset($_SESSION['sc_session'][$sc_init]['control_faltas']['pivot_order_sort']);
       unset($_SESSION['sc_session'][$sc_init]['control_faltas']['pivot_order_start']);
       unset($_SESSION['sc_session'][$sc_init]['control_faltas']['pivot_tabular']);
   }
   if ($opc_ret == "resumo" && !empty($this->summ_fields))
   {
       $summ_fields = explode("@?@", $this->summ_fields);
       if(isset($_SESSION['sc_session'][$sc_init]['control_faltas']['summarizing_fields_display'][ $sel_groupby ]))
       {
           foreach ($_SESSION['sc_session'][$sc_init]['control_faltas']['summarizing_fields_display'][ $sel_groupby ] as $i_sum => $d_sum)
           {
               $_SESSION['sc_session'][$sc_init]['control_faltas']['summarizing_fields_display'][ $sel_groupby ][$i_sum]['display'] = in_array($i_sum, $summ_fields);
           }
       }
       $_SESSION['sc_session'][$sc_init]['control_faltas']['summarizing_fields_order'][ $sel_groupby ] = $summ_fields;
       $_SESSION['sc_session'][$sc_init]['control_faltas']['summarizing_drill_down'] = $this->drill_down;
   }
?>
    <script language="javascript"> 
<?php
...
?>
 <script language="javascript" type="text/javascript">
  function scSubmitGroupBy(sPos) {
   $.ajax({
    type: "POST",
    url: "control_faltas_sel_groupby.php",
    data: {
...
<?php
if ($this->restore && $bSummaryPage && isset($_SESSION['sc_session'][$sc_init]['control_faltas']['dados_orig_gb']['res']['summarizing_fields_control']))
{
    $aOptions = array();
    foreach ($_SESSION['sc_session'][$sc_init]['control_faltas']['dados_orig_gb']['res']['summarizing_fields_control'] as $quebra_nome => $_arr_fields)
    {
...
elseif (!$this->restore && $bSummaryPage && isset($_SESSION['sc_session'][$sc_init]['control_faltas']['summarizing_fields_control']))
{
    $aOptions = array();
    foreach ($_SESSION['sc_session'][$sc_init]['control_faltas']['summarizing_fields_control'] as $quebra_nome => $_arr_fields)
    {
        if(is_array($_arr_fields))
        {
            foreach ($_arr_fields as $_key_total => $d_field)
...
{
  $max_height = 1;
  if ($this->restore && isset($_SESSION['sc_session'][$sc_init]['control_faltas']['dados_orig_gb']['res']['summarizing_fields_display']))
  {
    foreach ($_SESSION['sc_session'][$sc_init]['control_faltas']['dados_orig_gb']['res']['summarizing_fields_display'] as $_quebra_nome => $_i_sum)
    {
  ...
  elseif (!$this->restore && isset($_SESSION['sc_session'][$sc_init]['control_faltas']['summarizing_fields_display']))
  {
    foreach ($_SESSION['sc_session'][$sc_init]['control_faltas']['summarizing_fields_display'] as $_quebra_nome => $_i_sum)
    {
      if (count($_i_sum) > $max_height)
      {
          $max_height = count($_i_sum);
...
<?php
if ($bSummaryPage && $_SESSION['sc_session'][$sc_init]['control_faltas']['SC_Ind_Groupby'] == "sc_free_group_by") {
?>
   if (fieldList == "") {
...
<?php
    ...
    $iTabCount            = 0;
    foreach ($_SESSION['sc_session'][$sc_init]['control_faltas']['SC_All_Groupby'] as $QB => $Tp)
    {
        if (!in_array($QB, $_SESSION['sc_session'][$sc_init]['control_faltas']['SC_Groupby_hide']))
        {
...
<?php
     if (!in_array("AREA", $_SESSION['sc_session'][$sc_init]['control_faltas']['SC_Groupby_hide']))
     {
         $this->static_gb_ok = "sc_id_sel_groupby_AREA";
        if ($groupby_atual == "AREA")
        {
            $check = " checked";
            $bStartFree = false;
            $this->static_gb_ok = "sc_id_sel_groupby_AREA";
...
?>
      <div>
      <input type="radio" class="scAppDivToolbarInput sc_ui_gby_selected" onclick='scShowTotalFields();change_actual_static("sc_id_sel_groupby_AREA");display_btn_restore_gb();' name="sel_groupby" value="AREA" id="sc_id_sel_groupby_AREA"<?php echo $check ?> style='display:' />
      <label for="sc_id_sel_groupby_AREA">
        Area
<?php
    $prep_labels = array();
    $prep_labels[] = "AREA";
...
?>
<?php
     if (!in_array("USUARIO_SANCION", $_SESSION['sc_session'][$sc_init]['control_faltas']['SC_Groupby_hide']))
     {
        if ($groupby_atual == "USUARIO_SANCION")
        {
            $check = " checked";
            $bStartFree = false;
            $this->static_gb_ok = "sc_id_sel_groupby_USUARIO_SANCION";
        }
        else
        {
            $check = "";
...
?>
      <div>
      <input type="radio" class="scAppDivToolbarInput sc_ui_gby_selected" onclick='scShowTotalFields();change_actual_static("sc_id_sel_groupby_USUARIO_SANCION");display_btn_restore_gb();' name="sel_groupby" value="USUARIO_SANCION" id="sc_id_sel_groupby_USUARIO_SANCION"<?php echo $check ?> style='display:' />
      <label for="sc_id_sel_groupby_USUARIO_SANCION">
        USUARIO_SANCION
<?php
    $prep_labels = array();
    $prep_labels[] = "USUARIO SANCION";
?>
        <span>
        <?php echo implode(', ', $prep_labels);?>
        </span>
...
<?php
   ...
   if ($bSummaryPage)
   {
       ...
       if (!isset($_SESSION['sc_session'][$sc_init]['control_faltas']['summarizing_fields_control'])) {
           $arr_filds = array();
       }
       else {
           $arr_filds = ($this->restore) ? $_SESSION['sc_session'][$sc_init]['control_faltas']['dados_orig_gb']['res']['summarizing_fields_control'] : $_SESSION['sc_session'][$sc_init]['control_faltas']['summarizing_fields_control'];
       }
    if (isset($arr_filds))
    {
?>
...
<?php
          if (!isset($_SESSION['sc_session'][$sc_init]['control_faltas']['summarizing_fields_display'])) {
              $arr_disp = array();
          }
          else {
                 $arr_disp = ($this->restore) ? $_SESSION['sc_session'][$sc_init]['control_faltas']['dados_orig_gb']['res']['summarizing_fields_display'] : $_SESSION['sc_session'][$sc_init]['control_faltas']['summarizing_fields_display'];
          }
          if (!isset($_SESSION['sc_session'][$sc_init]['control_faltas']['summarizing_fields_order'])) {
              $arr_ord = array();
          }
          else {
              $arr_ord  = ($this->restore) ? $_SESSION['sc_session'][$sc_init]['control_faltas']['dados_orig_gb']['res']['summarizing_fields_order']   : $_SESSION['sc_session'][$sc_init]['control_faltas']['summarizing_fields_order'];
          }
...
<script type="text/javascript">
   ...
   if ($this->restore) {
       $oJson = new Services_JSON();
        ob_end_clean();
        if ($_SESSION['sc_session'][$sc_init]['control_faltas']['dados_orig_gb']['all']['SC_Ind_Groupby'] == "sc_free_group_by") {
           $this->Arr_result['run_js'][] = "scShowFree()";
        } else {
           $this->Arr_result['set_var'][] = array('var' => 'actual_static_gb', 'value' => $_SESSION['sc_session'][$sc_init]['control_faltas']['dados_orig_gb']['all']['SC_Ind_Groupby']);
...
<?php
   $disp_rest = "none";
   foreach ($_SESSION['sc_session'][$sc_init]['control_faltas']['dados_orig_gb'] as $tp => $fields) {
       foreach ($fields as $name => $data) {
           if (isset($_SESSION['sc_session'][$sc_init]['control_faltas'][$name])) {
               if ($tp == "all" && ($name != "SC_Gb_Free_cmp" || $_SESSION['sc_session'][$sc_init]['control_faltas']['SC_Ind_Groupby'] == "sc_free_group_by")) {
                   if ($data != $_SESSION['sc_session'][$sc_init]['control_faltas'][$name]) {
                       $disp_rest = "";
                       break;
                   }
               }
               elseif ($tp == "res" && $bSummaryPage && $data != $_SESSION['sc_session'][$sc_init]['control_faltas'][$name]) {
                   $disp_rest = "";
                   break;
               }
           }
