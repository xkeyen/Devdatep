...
class control_faltas_total
{
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['control_faltas']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['control_faltas']['campos_busca']))
      { 
   ...
   //----- 
   function Calc_resumo_AREA($destino_resumo, $res_export=false, $ind_compara="")
   {
      global $nm_lang;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('area' => "f.area_id");
      $cmps_quebra_atual = array("area");
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['SC_force_fiels_gb']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['SC_force_fiels_gb']))
      {
          $cmps_quebra_atual = $_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['SC_force_fiels_gb'];
      }
      ...
      {
          if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['SC_clear_total']) && $_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['SC_clear_total']) {
      ...
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['SC_clear_total']);
      $ind_cmps  = 2;
      $ind_alias = "1";
      $cmp_dim   = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          ...
          $this->Res_Totaliza_AREA($ind_qb, $cmp_gb, $arr_tots . $arr_compara, $group, $join, $cmps_gb, $cmps_gb1, $cmps_gb2, $cmps_gbS, $cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export);
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }

   function Res_Totaliza_AREA($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export)
   {
      $sc_having = ((isset($parms_sub_sel['having']))) ? "  having " . $parms_sub_sel['having'] : "";
      $Tem_estat_manual = false;
      $where_ok = $this->sc_where_atual;
      $cmp_sql_tp_num = array('tiempo_transcurrido' => 'N');
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
         $cmd_simp = "select count(*), count(f.area_id)#@#cmps_quebras#@# from " . $this->Ini->nm_tabela . " " . $where_ok;
         $comando  = "select count(*), count(SC_metric1)#@#cmps_quebras#@# from (";
         $comando .= "select f.area_id as SC_metric1, " . $cmps_quebras1 . " from " . $this->Ini->nm_tabela . " " .  $where_ok . ") SC_sel1 INNER JOIN (";
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['area']['reg'] = "S";
      $format_dimensions['area']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              $SC_prep = $this->Ini->Get_format_dimension($Ind_sql, 'AREA', $Cada_dim, $rt, $format_dimensions[$Cada_dim]['reg'], $format_dimensions[$Cada_dim]['msk']);
              $SC_orig = $Cada_dim . "_orig";
              $SC_graf = "val_grafico_" . $Cada_dim;
              $$Cada_dim = $SC_prep['fmt'];
              $$SC_orig = $SC_prep['orig'];
              if ($Cada_dim == "area") {
                  $this->Lookup->lookup_AREA_area($$Cada_dim,  $area);
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
          ...
          if ($Tem_estat_manual)
          {
              ...
              $vl_statistic = $this->Calc_statist_manual_AREA($where_ok);
   ...
   //----- 
   function Calc_resumo_USUARIO_SANCION($destino_resumo, $res_export=false, $ind_compara="")
   {
      global $nm_lang;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['sql_tot_res']);
      ...
      $this->sc_where_atual  = $_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['where_pesq'];
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('usuario_sancion' => "g3.general_name");
      $cmps_quebra_atual = array("usuario_sancion");
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['SC_force_fiels_gb']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['SC_force_fiels_gb']))
      {
          $cmps_quebra_atual = $_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['SC_force_fiels_gb'];
      }
      ...
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          ...
          $arr_compara = "[" . $ind_compara . "]";
          $this->Res_Totaliza_USUARIO_SANCION($ind_qb, $cmp_gb, $arr_tots . $arr_compara, $group, $join, $cmps_gb, $cmps_gb1, $cmps_gb2, $cmps_gbS, $cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export);
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
   ...
   function Res_Totaliza_USUARIO_SANCION($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export)
   {
      $sc_having = ((isset($parms_sub_sel['having']))) ? "  having " . $parms_sub_sel['having'] : "";
      $Tem_estat_manual = false;
      $where_ok = $this->sc_where_atual;
      ...
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
         $cmd_simp = "select count(*), avg(DATEDIFF(f.sanction_date,f.fault_practitioner_date))#@#cmps_quebras#@# from " . $this->Ini->nm_tabela . " " . $where_ok;
         $comando  = "select count(*), avg(SC_metric1)#@#cmps_quebras#@# from (";
         $comando .= "select DATEDIFF(f.sanction_date,f.fault_practitioner_date) as SC_metric1, " . $cmps_quebras1 . " from " . $this->Ini->nm_tabela . " " .  $where_ok . ") SC_sel1 INNER JOIN (";
         $comando .= "select " . $cmps_quebras2 . " from " . $this->Ini->nm_tabela . " " .  $where_ok . " group by " . $group . $sc_having . ") SC_sel2 ";
         $comando .= " ON " . $join;
      } 
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_sybase))
      ...
      $format_dimensions = array();
      $format_dimensions['usuario_sancion']['reg'] = "S";
      $format_dimensions['usuario_sancion']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              $prep_look  = $Cada_dim . "_SC_look";
              $$prep_look = $rt->fields[$Ind_sql];
              $SC_prep = $this->Ini->Get_format_dimension($Ind_sql, 'USUARIO_SANCION', $Cada_dim, $rt, $format_dimensions[$Cada_dim]['reg'], $format_dimensions[$Cada_dim]['msk']);
              $SC_orig = $Cada_dim . "_orig";
              $SC_graf = "val_grafico_" . $Cada_dim;
              $$Cada_dim = $SC_prep['fmt'];
              $$SC_orig = $SC_prep['orig'];
              if ($Cada_dim == "usuario_sancion") {
          nmgp_Form_Num_Val($$Cada_dim, $_SESSION['scriptcase']['reg_conf']['grup_val'], $_SESSION['scriptcase']['reg_conf']['dec_val'], "0", "S", "2", "", "V:" . $_SESSION['scriptcase']['reg_conf']['monet_f_pos'] . ":" . $_SESSION['scriptcase']['reg_conf']['monet_f_neg'], $_SESSION['scriptcase']['reg_conf']['simb_neg'], $_SESSION['scriptcase']['reg_conf']['unid_mont_group_digit']) ; 
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
          ...
          {
              $where_ok = (empty($this->sc_where_atual)) ? " where " . $sql_where : $this->sc_where_atual . " and " . $sql_where;
              $vl_statistic = $this->Calc_statist_manual_USUARIO_SANCION($where_ok);
              foreach ($vl_statistic as $ind => $val)
              {
                  $rt->fields[$ind] = $val;
              }
   ...
   //---- 
   function quebra_geral_AREA($res_limit=false, $res_export=false)
   {
      global $nada, $nm_lang ;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), count(f.area_id) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['where_pesq']; 
      } 
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*), count(f.area_id) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['tot_geral'][2] = $rt->fields[1]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['contr_total_geral'] = "OK";
   } 

   //---- 
   function quebra_geral_USUARIO_SANCION($res_limit=false, $res_export=false)
   {
      global $nada, $nm_lang ;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['contr_total_geral'] == "OK") 
      { 
      ...
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), avg(DATEDIFF(f.sanction_date,f.fault_practitioner_date)) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['where_pesq']; 
      } 
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*), avg(DATEDIFF(f.sanction_date,f.fault_practitioner_date)) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['where_pesq']; 
   ...
   //-----  area
   function quebra_area_AREA($area, $arg_sum_area) 
   {
      global $tot_area;
      $tot_area = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      ...
      { 
         $nm_comando .= " where f.area_id" . $arg_sum_area ; 
      } 
      else 
      { 
         $nm_comando .= " and f.area_id" . $arg_sum_area ; 
      } 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['opcao'] == "pdf" && isset($_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content']) && $_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content'] == 'Y') {
          $tot_area[0] = NM_encode_input(sc_strip_script($area));
      }
      else {
          $tot_area[0] = sc_strip_script($area) ; 
      }
      $tot_area[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_area[2] = (string)$rt->fields[1]; 
      $rt->Close(); 
   } 

   //-----  usuario_sancion
   function quebra_usuario_sancion_USUARIO_SANCION($usuario_sancion, $arg_sum_usuario_sancion) 
   {
      global $tot_usuario_sancion;
      $tot_usuario_sancion = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), avg(DATEDIFF(f.sanction_date,f.fault_practitioner_date)) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['where_pesq']; 
      } 
      ...
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['where_pesq'])) 
      { 
         $nm_comando .= " where g3.general_name" . $arg_sum_usuario_sancion ; 
      } 
      else 
      { 
         $nm_comando .= " and g3.general_name" . $arg_sum_usuario_sancion ; 
      } 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
      ...
      $tot_usuario_sancion[0] = NM_encode_input(sc_strip_script($usuario_sancion)) ; 
      $tot_usuario_sancion[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_usuario_sancion[2] = (string)$rt->fields[1]; 
      $rt->Close(); 
   } 
