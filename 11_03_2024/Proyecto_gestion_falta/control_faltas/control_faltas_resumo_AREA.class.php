   ...
   if ($this->Ini->Embutida_iframe) {
        ...
        $this->SC_APP_info = [
            ...
            'metric' => [
                'area-C' => [
                    ...
                    'is_rating' => '' != '',
                    'rating_function' => '',
                    'format_function' => 'aux_format_area_C',
                    'css_class' => "css_area_cnt_field",
            ...
            'options' => [
                ...
                'order_initial_rule' => '',
                'order_metric_apply_to_dimensions' => ['AREA'],
    ...
    function info_saveSessionDefaultInfo()
    {
        if (!isset($_SESSION['sc_session'] [$this->Ini->sc_page] ['control_faltas'] ['last_displayed_group_by'])) {
            $_SESSION['sc_session'] [$this->Ini->sc_page] ['control_faltas'] ['last_displayed_group_by'] = 'AREA';
            $this->SC_APP_info['options'] ['starting_group_by'] = true;
        }
        if ('AREA' != $_SESSION['sc_session'] [$this->Ini->sc_page] ['control_faltas'] ['last_displayed_group_by']) {
            $_SESSION['sc_session'] [$this->Ini->sc_page] ['control_faltas'] ['last_displayed_group_by'] = 'AREA';
            $this->SC_APP_info['options'] ['starting_group_by'] = true;
        }
    ...
    function info_loadSessionInfo()
    {
        ...
        foreach ($_SESSION['sc_session'] [$this->Ini->sc_page] ['control_faltas'] ['summarizing_fields_order'] ['AREA'] as $metricIndex) {
            if ($_SESSION['sc_session'] [$this->Ini->sc_page] ['control_faltas'] ['summarizing_fields_display'] ['AREA'] [$metricIndex] ['display']) {
...
SCEOT;
    ...
    function aux_format_area_C($value)
    {
        $isNegative = 0 > $value;
        $suffix = '';
...
SCEOT;
   ...
   function monta_resumo($b_export = false)
   {
     ...
     { 
      ...
      {
            if ($this->aux_hasXAxysDimensionField()) {
                $this->array_line_area = $_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['arr_total_line']['area'];
            }
          $this->array_total_area = $_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['arr_total']['area'];
      ...
      else
      {
          ...
          $_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['arr_total']['area'] = $this->array_total_area;
            if ($this->aux_hasXAxysDimensionField() && $this->SC_APP_info['options'] ['display_total_column']) {
                $_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['arr_total_line']['area'] = $this->array_line_area;
   ...
   function completeMatrix()
   {
       ...
       //-----
       $this->comp_sum = array(
           2 => "AREA (" .  $this->Ini->Nm_lang['lang_btns_smry_msge_cnt'] . ")",
       );
       ...
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['summarizing_fields_order']['AREA']))
       {
           foreach ($this->comp_sum as $i_sum => $l_sum)
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['summarizing_fields_order']['AREA'][] = $i_sum;
           }
       }
       else
       {
           $this->comp_sum_order = $_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['summarizing_fields_order']['AREA'];
       }
       ...
       $this->comp_sum_display = array(
           ...
           foreach ($this->comp_sum as $i_sum => $l_sum)
           {
               if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['summarizing_fields_display']['AREA'][$i_sum]))
               {
                   $_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['summarizing_fields_display']['AREA'][$i_sum] = array('label' => $l_sum, 'display' => $this->comp_sum_display[$i_sum]);
               }
               else
               {
                   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['summarizing_fields_display']['AREA'][$i_sum]['label']))
                   {
                       $_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['summarizing_fields_display']['AREA'][$i_sum]['label'] = $l_sum;
                   }
                   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['summarizing_fields_display']['AREA'][$i_sum]['display']))
                   {
                       $_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['summarizing_fields_display']['AREA'][$i_sum]['display'] = $this->comp_sum_display[$i_sum];
                   }
               }
           }
           foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['summarizing_fields_display']['AREA'] as $i_sum => $d_sum)
           {
       ...
       //-----
       foreach ($this->array_total_area as $i_area => $d_area) {
           if (!isset($i_area, $this->comp_label[0]) || !in_array($i_area, $this->comp_label[0], true)) {
               $this->comp_index[0][ $d_area[3] ] = $d_area[2];
               $this->comp_label[0][ $d_area[3] ] = $d_area[2];
           }
       }
     ...
     if (isset($this->comp_label[0])) {
       foreach ($this->comp_label[0] as $i_area => $l_area) {
           if (isset($this->array_total_area[$i_area])) {
               $this->comp_group[$i_area] = array();
           }
       }
     }
   ...
   function buildChart()
   {
       ...
       $this->comp_chart_config = array(
           '0|1' => array(
               'title'    => $this->getChartText(strip_tags("AREA (" .  $this->Ini->Nm_lang['lang_btns_smry_msge_cnt'] . ")")),
               'show_sub' => true,
               'subtitle' => "",
               'field_x'  => 'AREA',
               'field_y'  => $this->comp_sum_nm[2],
               'label_x'  => $this->getChartText(strip_tags($Lab_area)),
               'label_y'  => $this->getChartText(strip_tags("AREA (" .  $this->Ini->Nm_lang['lang_btns_smry_msge_cnt'] . ")")),
...
if($_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['proc_pdf'] || $_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['proc_pdf_vert']){ 
   ...
   function totaliza()
   {
      ...
      $where_compara   = "";
      $this->Total->Calc_resumo_AREA("res", $this->NM_export, 1);
      if ((isset($_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['where_pesq_compara'])    && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['where_pesq_compara'])) 
       || (isset($_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['where_dyn_compara'])     && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['control_faltas']['where_dyn_compara'])) )
       {
          ...
          $this->Total->Calc_resumo_AREA("res", $this->NM_export, 2);
      ...
      if (is_array($this->array_total_area)) {
          ksort($this->array_total_area);
      }
...
//--- 
   ...
   //---- 
   function adiciona_registro($cnt_area, $quebra_area, $quebra_area_orig)
   {
      //----- AREA
      if (!isset($this->array_total_area[$quebra_area_orig]))
      {
         $this->array_total_area[$quebra_area_orig][0] = 1;
         $this->array_total_area[$quebra_area_orig][1] = 1;
         $this->array_total_area[$quebra_area_orig][2] = $quebra_area;
         $this->array_total_area[$quebra_area_orig][3] = $quebra_area_orig;
      }
      else
      {
         $this->array_total_area[$quebra_area_orig][0]++;
         $this->array_total_area[$quebra_area_orig][1] ++;
      }
   }
   ...
   {
      if (isset($this->array_total_area))
      {
         $arr_temp_0 = $this->array_total_area;
         foreach ($this->array_total_area as $campo_area => $dados_area)
         {
            $ind_exists  = "";
            $ind_missing = "";
            if (!isset($this->array_total_area[$campo_area][1])) {
                $ind_exists  = 2;
                $ind_missing = 1;
            }
            if (!isset($this->array_total_area[$campo_area][2])) {
                $ind_exists  = 1;
                $ind_missing = 2;
            }
            if (!empty($ind_missing)) {
                $arr_temp_0[$campo_area][$ind_missing][0] = 0;
                $arr_temp_0[$campo_area][$ind_missing][1] = 0;
                $arr_temp_0[$campo_area][$ind_missing][2] = $dados_area[$ind_exists][2];
                $arr_temp_0[$campo_area][$ind_missing][3] = $dados_area[$ind_exists][3];
            }
            ksort($arr_temp_0[$campo_area]);
            $this->array_total_area[$campo_area] = $arr_temp_0[$campo_area];
         } 
      } 
   }
...
//
   function nm_acumula_resumo($nm_tipo="resumo")
   {
     ...
     $campos_order = "";
     $format       = $this->Ini->Get_Gb_date_format('AREA', 'area');
     $campos_order = $this->Ini->Get_date_order_groupby("f.area_id", 'asc', $format, $campos_order);
...
// 
   function nm_acum_res_unit($rs_res, $nm_tipo="resumo")
   {
            ...
            if ($nm_tipo == "resumo")
            {
                $this->adiciona_registro($this->area, $this->area, $this->area_orig);
            }
   }
//
