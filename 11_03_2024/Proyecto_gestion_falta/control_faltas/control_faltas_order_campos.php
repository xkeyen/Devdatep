...
<?php
   ...
   $Ord_Cmp = new control_faltas_Ord_cmp(); 
   $Ord_Cmp->Ord_cmp_init();
   
class control_faltas_Ord_cmp
{
function Ord_cmp_init()
{
  global $sc_init, $path_img, $path_btn, $use_alias, $tab_ger_campos, $tab_def_campos, $tab_def_seq, $tab_labels, $embbed, $tbar_pos, $_POST, $_GET;
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['control_faltas']['ordem_select_orig']))
   {
       $_SESSION['sc_session'][$sc_init]['control_faltas']['ordem_select_orig'] = array();
   }
   ...
   $tab_labels["colaborador"]   = (isset($_SESSION['sc_session'][$sc_init]['control_faltas']['labels']["colaborador"])) ? $_SESSION['sc_session'][$sc_init]['control_faltas']['labels']["colaborador"] : "COLABORADOR";
   ...
   $tab_labels["desc_falta"]   = (isset($_SESSION['sc_session'][$sc_init]['control_faltas']['labels']["desc_falta"])) ? $_SESSION['sc_session'][$sc_init]['control_faltas']['labels']["desc_falta"] : "DESC FALTA";
   ...
   $tab_labels["fecha_falta"]   = (isset($_SESSION['sc_session'][$sc_init]['control_faltas']['labels']["fecha_falta"])) ? $_SESSION['sc_session'][$sc_init]['control_faltas']['labels']["fecha_falta"] : "FECHA FALTA";
   ...
   $tab_labels["usuario_registro"]   = (isset($_SESSION['sc_session'][$sc_init]['control_faltas']['labels']["usuario_registro"])) ? $_SESSION['sc_session'][$sc_init]['control_faltas']['labels']["usuario_registro"] : "USUARIO REGISTRO";
   ...
   $tab_labels["perfil_r"]   = (isset($_SESSION['sc_session'][$sc_init]['control_faltas']['labels']["perfil_r"])) ? $_SESSION['sc_session'][$sc_init]['control_faltas']['labels']["perfil_r"] : "PERFIL R";
   ...
   $tab_labels["fecha_registro"]   = (isset($_SESSION['sc_session'][$sc_init]['control_faltas']['labels']["fecha_registro"])) ? $_SESSION['sc_session'][$sc_init]['control_faltas']['labels']["fecha_registro"] : "FECHA REGISTRO";
   $tab_ger_campos['usuario_sancion'] = "on";
   if ($use_alias == "S")
   {
       $tab_def_campos['usuario_sancion'] = "usuario_sancion";
   }
   else
   {
       $tab_def_campos['usuario_sancion'] = "g3.general_name";
   }
   $tab_labels["usuario_sancion"]   = (isset($_SESSION['sc_session'][$sc_init]['control_faltas']['labels']["usuario_sancion"])) ? $_SESSION['sc_session'][$sc_init]['control_faltas']['labels']["usuario_sancion"] : "USUARIO SANCION";
   ...
   $tab_labels["perfil_s"]   = (isset($_SESSION['sc_session'][$sc_init]['control_faltas']['labels']["perfil_s"])) ? $_SESSION['sc_session'][$sc_init]['control_faltas']['labels']["perfil_s"] : "PERFIL S";
   $tab_ger_campos['fecha_sancion'] = "on";
   if ($use_alias == "S")
   {
       $tab_def_campos['fecha_sancion'] = "fecha_sancion";
   }
   else
   {
       $tab_def_campos['fecha_sancion'] = "f.sanction_date";
   }
   $tab_labels["fecha_sancion"]   = (isset($_SESSION['sc_session'][$sc_init]['control_faltas']['labels']["fecha_sancion"])) ? $_SESSION['sc_session'][$sc_init]['control_faltas']['labels']["fecha_sancion"] : "FECHA SANCION";
   $tab_ger_campos['tiempo_transcurrido'] = "on";
   if ($use_alias == "S")
   {
       $tab_def_campos['tiempo_transcurrido'] = "tiempo_transcurrido";
   }
   else
   {
       $tab_def_campos['tiempo_transcurrido'] = "DATEDIFF(f.sanction_date,f.fault_practitioner_date)";
   }
   $tab_labels["tiempo_transcurrido"]   = (isset($_SESSION['sc_session'][$sc_init]['control_faltas']['labels']["tiempo_transcurrido"])) ? $_SESSION['sc_session'][$sc_init]['control_faltas']['labels']["tiempo_transcurrido"] : "TIEMPO TRANSCURRIDO";
   ...
   if (isset($_SESSION['sc_session'][$sc_init]['control_faltas']['php_cmp_sel']) && !empty($_SESSION['sc_session'][$sc_init]['control_faltas']['php_cmp_sel']))
   {
       foreach ($_SESSION['sc_session'][$sc_init]['control_faltas']['php_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
       {
           if ($NM_cada_opc == "off")
           {
              $tab_ger_campos[$NM_cada_field] = "none";
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['control_faltas']['ordem_select']))
   {
       $_SESSION['sc_session'][$sc_init]['control_faltas']['ordem_select'] = array();
   }
...
function Sel_processa_out_sel($campos_sel)
{
   ...
   $_SESSION['sc_session'][$sc_init]['control_faltas']['ordem_select'] = array();
   $_SESSION['sc_session'][$sc_init]['control_faltas']['ordem_grid']   = "";
   $_SESSION['sc_session'][$sc_init]['control_faltas']['ordem_cmp']    = "";
   foreach ($campos_sel as $campo_sort)
   {
       ...
       if (isset($tab_def_campos[$campo]))
       {
           ...
           if (isset($_SESSION['sc_session'][$sc_init]['control_faltas']['ordem_quebra']))
           {
               foreach($_SESSION['sc_session'][$sc_init]['control_faltas']['ordem_quebra'] as $campo_quebra => $resto) 
               {
                   foreach($resto as $sqldef => $ordem_ant) 
                   {
                       if ($sqldef == $tab_def_campos[$campo]) 
                       { 
                           $_SESSION['sc_session'][$sc_init]['control_faltas']['ordem_quebra'][$campo][$sqldef] = $ordem;
           ...
           if (!$Ordem_tem_quebra)
           {
               $_SESSION['sc_session'][$sc_init]['control_faltas']['ordem_select'][$tab_def_campos[$campo]] = $ordem;
           }   
       }
   }
?>
...
?>
  function scSubmitOrderCampos(sPos, sType) {
   ...
   return new Promise(function(resolve, reject) {$.ajax({
    type: "POST",
    url: "control_faltas_order_campos.php",
    data: {
...
<?php
   ...
   $arr_order = ($this->restore) ? $_SESSION['sc_session'][$sc_init]['control_faltas']['ordem_select_orig'] : $_SESSION['sc_session'][$sc_init]['control_faltas']['ordem_select'];
...
<?php
  if (isset($_SESSION['sc_session'][$sc_init]['control_faltas']['ordem_select']) && $_SESSION['sc_session'][$sc_init]['control_faltas']['ordem_select'] != $_SESSION['sc_session'][$sc_init]['control_faltas']['ordem_select_orig']) {
      $disp_rest = "";
  }
   if (!$embbed)
   {
