...
function ajax_navigate(opc, parm)
{
    ...
    return new Promise(function(resolve, reject) {$.ajax({
     ...
     .done(function(jsonNavigate) {
        ...
        if (!SC_Proc_Mobile) {
            document.getElementById('nmsc_iframe_liga_A_grid_agenda_tecnicos').src = 'NM_Blank_Page.htm';
            document.getElementById('nmsc_iframe_liga_E_grid_agenda_tecnicos').src = 'NM_Blank_Page.htm';
            document.getElementById('nmsc_iframe_liga_D_grid_agenda_tecnicos').src = 'NM_Blank_Page.htm';
            document.getElementById('nmsc_iframe_liga_B_grid_agenda_tecnicos').src = 'NM_Blank_Page.htm';
            document.getElementById('nmsc_iframe_liga_A_grid_agenda_tecnicos').style.height = '0px';
            document.getElementById('nmsc_iframe_liga_E_grid_agenda_tecnicos').style.height = '0px';
            document.getElementById('nmsc_iframe_liga_D_grid_agenda_tecnicos').style.height = '0px';
            document.getElementById('nmsc_iframe_liga_B_grid_agenda_tecnicos').style.height = '0px';
            document.getElementById('nmsc_iframe_liga_A_grid_agenda_tecnicos').style.width  = '0px';
            document.getElementById('nmsc_iframe_liga_E_grid_agenda_tecnicos').style.width  = '0px';
            document.getElementById('nmsc_iframe_liga_D_grid_agenda_tecnicos').style.width  = '0px';
            document.getElementById('nmsc_iframe_liga_B_grid_agenda_tecnicos').style.width  = '0px';
        }
        if (oResp["setValue"]) {
...
{
    if ($("#id_dyn_search_fields").prop("disabled") == true) {
        return;
    }
    var parm = str_field;
    if (parm == "")
...
function ajax_ch_bi_dyn_search(field, ind, parm, str_origem)
{
    nmAjaxProcOn();
    $.ajax({
      type: "POST",
      url: "index.php",
      data: "nmgp_opcao=ajax_ch_bi_dyn_search&script_case_init=" + document.Fdyn_search.script_case_init.value + "&field=" + field + "&parm=" + parm + "&origem=" + str_origem
     })
     .done(function(jsonDyn_ch_bi) {
        var i, oResp;
        Tst_integrid = jsonDyn_ch_bi.trim();
        if ("{" != Tst_integrid.substr(0, 1)) {
            nmAjaxProcOff();
            alert (jsonDyn_ch_bi);
            return;
        }
        eval("oResp = " + jsonDyn_ch_bi);
        if (oResp["ss_time_out"]) {
            nmAjaxProcOff();
            nm_move();
        }
        if (oResp["dyn_ch_bi"]) {
          for (i = 0; i < oResp["dyn_ch_bi"].length; i++) {
               $('#ID_bi_dados_' + field + '_' + ind).html(oResp["dyn_ch_bi"][i]);
          }
        }
        $('#ID_bi_dados_' + field + '_' + ind).css('display','');
        $('#ID_nm_dados_' + field + '_' + ind).css('display','none');
        nmAjaxProcOff();
    });
}
...
function ajax_ch_bi_grid_search(field, ind, parm, str_origem)
{
    nmAjaxProcOn();
    $.ajax({
      type: "POST",
      url: "index.php",
      data: "nmgp_opcao=ajax_ch_bi_dyn_search&script_case_init=" + document.Fgrid_search.script_case_init.value + "&field=" + field + "&parm=" + parm + "&origem=" + str_origem
     })
     .done(function(jsonDyn_ch_bi) {
        var i, oResp;
        Tst_integrid = jsonDyn_ch_bi.trim();
        ...
        if (oResp["dyn_ch_bi"]) {
          for (i = 0; i < oResp["dyn_ch_bi"].length; i++) {
               $('#ID_bi_dados_grid_' + field + '_' + ind).html(oResp["dyn_ch_bi"][i]);
          }
        }
        $('#ID_bi_dados_grid_' + field + '_' + ind).css('display','');
        $('#ID_nm_dados_grid_' + field + '_' + ind).css('display','none');
        nmAjaxProcOff();
    });
}
