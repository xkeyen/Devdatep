<?php
   ...
   if (!isset($_SESSION['sc_session']))
   {
       ...
       if (is_file($root . $_SESSION['scriptcase']['grid_agenda_tecnicos']['glo_nm_path_imag_temp'] . "/sc_apl_default_prueba2.txt"))
       {
?>
...
function Sel_processa_out($sel_groupby)
{
   ...
   elseif ($sel_groupby != $groupby_atual || $Change_free_groupby)
   {
       ...
       unset($_SESSION['sc_session'][$sc_init]['grid_agenda_tecnicos']['pivot_tabular']);
           ...
           $tab_arr_groubby_cmp['fecha_inicial_scgb_19'] = array('cmp' => "fecha_inicial", 'ind' => 1, 'format' => "WEEK");
...
function Sel_processa_form()
{
   ...
   $arr_campos_free['fecha_inicial'] = "" . sprintf("" . $this->Nm_lang['lang_othr_cons_title_YYYYMM'] . "", "Fecha Inicial") . "";
   $arr_campos_free['fecha_inicial_scgb_19'] = "" . sprintf("" . $this->Nm_lang['lang_othr_cons_title_WEEK'] . "", "Fecha Inicial") . "";
   $arr_campos_free['valor_total'] = "Valor Total";
   $arr_campos_free['estado_actual'] = "Estado Actual";
   $arr_campos_free['id_tecnico'] = "Id Tecnico";
...
?>
<?php
     if (!in_array("ag_mensual", $_SESSION['sc_session'][$sc_init]['grid_agenda_tecnicos']['SC_Groupby_hide']))
     {
        if ($groupby_atual == "ag_mensual")
        {
            $check = " checked";
            $bStartFree = false;
            $this->static_gb_ok = "sc_id_sel_groupby_ag_mensual";
        }
        else
        {
            $check = "";
...
?>
      <div>
      <input type="radio" class="scAppDivToolbarInput sc_ui_gby_selected" onclick='scShowTotalFields();change_actual_static("sc_id_sel_groupby_ag_mensual");display_btn_restore_gb();' name="sel_groupby" value="ag_mensual" id="sc_id_sel_groupby_ag_mensual"<?php echo $check ?> style='display:' />
      <label for="sc_id_sel_groupby_ag_mensual">
        Mensual
<?php
    $prep_labels = array();
    $prep_labels[] = sprintf("" . $this->Nm_lang['lang_othr_cons_title_YYYYMM'] . "", "Fecha Inicial");
    $prep_labels[] = sprintf("" . $this->Nm_lang['lang_othr_cons_title_WEEK'] . "", "Fecha Inicial");
?>
        <span>
        <?php echo implode(', ', $prep_labels);?>
        </span>
