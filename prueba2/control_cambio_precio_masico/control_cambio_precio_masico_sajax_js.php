  ...
  function scAjaxUpdateErrors(sType)
  {
    ...
    if (oResp["errList"])
    {
      for (iFieldErrors = 0; iFieldErrors < oResp["errList"].length; iFieldErrors++)
      {
        ...
        if ("geral_control_cambio_precio_masico" == sTestField)
        {
...
//    return document.F1.elements[sFieldName].value.replace(/[+]/g, "__NM_PLUS__");
  ...
  function scMoveMasterDetail(widget)
  {
	...
	function scAjax_displayEmptyForm() {
        ...
        sc_hide_control_cambio_precio_masico_form();
	}
...
?>
  ...
  // ---------- Validate porcentaje
  function do_ajax_control_cambio_precio_masico_validate_porcentaje()
  {
    var nomeCampo_porcentaje = "porcentaje";
    var var_porcentaje = scAjaxGetFieldText(nomeCampo_porcentaje);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_control_cambio_precio_masico_validate_porcentaje(var_porcentaje, var_script_case_init, do_ajax_control_cambio_precio_masico_validate_porcentaje_cb);
  } // do_ajax_control_cambio_precio_masico_validate_porcentaje

  function do_ajax_control_cambio_precio_masico_validate_porcentaje_cb(sResp)
  {
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_control_cambio_precio_masico_validate_porcentaje_cb

  // ---------- Validate tipo
  function do_ajax_control_cambio_precio_masico_validate_tipo()
  {
    var nomeCampo_tipo = "tipo";
    var var_tipo = scAjaxGetFieldRadio(nomeCampo_tipo);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_control_cambio_precio_masico_validate_tipo(var_tipo, var_script_case_init, do_ajax_control_cambio_precio_masico_validate_tipo_cb);
  } // do_ajax_control_cambio_precio_masico_validate_tipo

  function do_ajax_control_cambio_precio_masico_validate_tipo_cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    sFieldValid = "tipo";
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_control_cambio_precio_masico_validate_tipo_cb
...
function scJs_sweetalert_params(params) {
  ...
  // ---------- Form
  function do_ajax_control_cambio_precio_masico_submit_form()
  {
    if (scEventControl_active("")) {
      setTimeout(function() { do_ajax_control_cambio_precio_masico_submit_form(); }, 500);
      return;
    }
    scAjaxHideMessage();
    ...
    scAjaxProcOn();
    x_ajax_control_cambio_precio_masico_submit_form(var_porcentaje, var_tipo, var_nm_form_submit, var_nmgp_url_saida, var_nmgp_opcao, var_nmgp_ancora, var_nmgp_num_form, var_nmgp_parms, var_script_case_init, var_csrf_token, do_ajax_control_cambio_precio_masico_submit_form_cb);
  } // do_ajax_control_cambio_precio_masico_submit_form

  function do_ajax_control_cambio_precio_masico_submit_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
...
?>
    ...
    scAjaxSetLabel();
    scAjaxSetReadonly();
    scAjaxAlert(do_ajax_control_cambio_precio_masico_submit_form_cb_after_alert);
  } // do_ajax_control_cambio_precio_masico_submit_form_cb
  function do_ajax_control_cambio_precio_masico_submit_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    scAjaxRedir();
  ...
  } // do_ajax_control_cambio_precio_masico_submit_form_cb_after_alert

  var scStatusDetail = {};

  function do_ajax_control_cambio_precio_masico_navigate_form()
  {
    if (scFormHasChanged) {
      scJs_confirm('<?php echo html_entity_decode($this->Ini->Nm_lang['lang_reload_confirm']) ?>', perform_ajax_control_cambio_precio_masico_navigate_form, function() {});
    } else {
      perform_ajax_control_cambio_precio_masico_navigate_form();
    }
  }

  function perform_ajax_control_cambio_precio_masico_navigate_form()
  {
    if (scRefreshTable())
    {
      return;
    }
  } // do_ajax_control_cambio_precio_masico_navigate_form

  var scMasterDetailParentIframe = "<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['control_cambio_precio_masico']['dashboard_info']['parent_widget'] ?>";
...
?>
  function do_ajax_control_cambio_precio_masico_navigate_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scAjaxSetBtnVars();
    $('.sc-js-ui-statusimg').css('display', 'none');
    scAjaxAlert(do_ajax_control_cambio_precio_masico_navigate_form_cb_after_alert);
  } // do_ajax_control_cambio_precio_masico_navigate_form_cb
  function do_ajax_control_cambio_precio_masico_navigate_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
<?php
...
{
?>
    do_ajax_control_cambio_precio_masico_restore_buttons();
<?php
}
?>
    if (hasJsFormOnload)
  ...
  } // do_ajax_control_cambio_precio_masico_navigate_form_cb_after_alert

  function sc_hide_control_cambio_precio_masico_form()
  {
