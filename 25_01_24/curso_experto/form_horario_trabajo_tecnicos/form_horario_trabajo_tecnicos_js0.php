...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_horario_trabajo_tecnicos"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
<?php
  if (isset($this->NM_ajax_info['masterValue']) && !empty($this->NM_ajax_info['masterValue']))
  {
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_horario_trabajo_tecnicos']['dashboard_info']['under_dashboard']) && $_SESSION['sc_session'][$this->Ini->sc_page]['form_horario_trabajo_tecnicos']['dashboard_info']['under_dashboard']) {
          echo "  var dbParentFrame = $(parent.document).find(\"[name='" . $_SESSION['sc_session'][$this->Ini->sc_page]['form_horario_trabajo_tecnicos']['dashboard_info']['parent_widget'] . "']\")";
...
?> 
function sc_btn_btn_prueba() 
{ 
    if (scEventControl_active_all()) {
      setTimeout(function() { sc_btn_btn_prueba(); }, 500);
      return;
    }
    check_all();
} 
function sc_btn_btn_prueba_cancel()
{
}
function sc_btn_btn_prueba_ok()
{
    check_all();
}
 NM_tp_critica(1);
...
function scForm_inline_submit(y, z) {
	Nm_Proc_Atualiz = true;
	do_ajax_form_horario_trabajo_tecnicos_submit_form(y, z);
