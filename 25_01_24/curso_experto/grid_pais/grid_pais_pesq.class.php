...
//-- ajax metodos ---
      ...
      if ($this->NM_ajax_opcao == 'autocomp_region')
      {
          $region = ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($_GET['q'])) ? sc_convert_encoding($_GET['q'], $_SESSION['scriptcase']['charset'], "UTF-8") : $_GET['q'];
          $nmgp_def_dados = $this->lookup_ajax_region($region);
          ob_end_clean();
          ob_end_clean();
          $count_aut_comp = 0;
          $resp_aut_comp  = array();
      ...
      if ($this->NM_ajax_opcao == 'autocomp_continente')
      {
          $continente = ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($_GET['q'])) ? sc_convert_encoding($_GET['q'], $_SESSION['scriptcase']['charset'], "UTF-8") : $_GET['q'];
          $nmgp_def_dados = $this->lookup_ajax_continente($continente);
          ob_end_clean();
          ob_end_clean();
          $count_aut_comp = 0;
          $resp_aut_comp  = array();
...
<HTML>
<HEAD>
 <TITLE><?php echo $this->Ini->Nm_lang['lang_othr_srch_title'] ?> pais</TITLE>
...
var nmdg_Form = "F1";
 ...
 function nm_save_form(pos)
 {
  ...
  var str_out = "";
  str_out += 'SC_region_cond#NMF#' + search_get_sel_txt('SC_region_cond') + '@NMF@';
  str_out += 'SC_region#NMF#' + search_get_text('SC_region') + '@NMF@';
  str_out += 'id_ac_region#NMF#' + search_get_text('id_ac_region') + '@NMF@';
  ...
  str_out += 'SC_continente_cond#NMF#' + search_get_sel_txt('SC_continente_cond') + '@NMF@';
  str_out += 'SC_continente#NMF#' + search_get_text('SC_continente') + '@NMF@';
  str_out += 'id_ac_continente#NMF#' + search_get_text('id_ac_continente') + '@NMF@';
...
<script type="text/javascript">
 $(function() {
   ...
   scClass = $("#id_ac_continente").attr('class').split(' ');
   scClass = scClass[ scClass.length-1 ];
   $("#id_ac_continente").autocomplete({
     minLength: 1,
     classes: { 'ui-autocomplete': scClass + 'Ac' },
     source: function (request, response) {
     $.ajax({
       ...
       data: {
          q: request.term,
          nmgp_opcao: "ajax_autocomp",
          nmgp_parms: "NM_ajax_opcao?#?autocomp_continente",
          max_itens: "10",
          cod_desc: "N",
          script_case_init: <?php echo $this->Ini->sc_page ?>
        },
     ...
     select: function (event, ui) {
       $("#SC_continente").val(ui.item.value);
       $(this).val(ui.item.label);
       event.preventDefault();
     },
     focus: function (event, ui) {
     ...
     change: function (event, ui) {
       if (null == ui.item) {
          $("#SC_continente").val( $(this).val() );
       }
     }
   });
   scClass = $("#id_ac_region").attr('class').split(' ');
   scClass = scClass[ scClass.length-1 ];
   $("#id_ac_region").autocomplete({
     minLength: 1,
     classes: { 'ui-autocomplete': scClass + 'Ac' },
     source: function (request, response) {
     $.ajax({
       ...
       data: {
          q: request.term,
          nmgp_opcao: "ajax_autocomp",
          nmgp_parms: "NM_ajax_opcao?#?autocomp_region",
          max_itens: "10",
          cod_desc: "N",
          script_case_init: <?php echo $this->Ini->sc_page ?>
        },
...
<?php
   ...
   {
      if ($this->Ini->Embutida_iframe || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pais']['dashboard_info']['compact_mode'])
      {
          return;
      }
...
<div class="scFilterHeader" style="height: 54px; padding: 17px 15px; box-sizing: border-box;margin: -1px 0px 0px 0px;width: 100%;">
    <div class="scFilterHeaderFont" style="float: left; text-transform: uppercase;"><?php echo $this->Ini->Nm_lang['lang_othr_srch_title'] ?> pais</div>
...
<?php
   ...
   {
      global 
             $region_cond, $region, $region_autocomp,
             $codigo_cond, $codigo, $codigo_autocomp,
             $nombre_cond, $nombre, $nombre_autocomp,
             $continente_cond, $continente, $continente_autocomp,
      ...
      { 
          ...
          $region = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pais']['campos_busca']['region']; 
          $region_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pais']['campos_busca']['region_cond']; 
          $codigo = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pais']['campos_busca']['codigo']; 
          $codigo_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pais']['campos_busca']['codigo_cond']; 
          ...
          $continente = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pais']['campos_busca']['continente']; 
          $continente_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pais']['campos_busca']['continente_cond']; 
          $this->NM_operador = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pais']['campos_busca']['NM_operador']; 
      } 
      if (!isset($region_cond) || empty($region_cond))
      {
         $region_cond = "qp";
      }
      ...
      if (!isset($continente_cond) || empty($continente_cond))
      {
         $continente_cond = "qp";
      }
      ...
      $str_hide_region = (in_array($region_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;
      ...
      $str_hide_continente = (in_array($continente_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;
      ...
      {
         $tmp_pos = (is_string($region)) ? strpos($region, "##@@") : false;
         if ($tmp_pos === false)
         { }
         else
         {
         $region = substr($region, 0, $tmp_pos);
         }
      }
      ...
      if (!isset($continente) || $continente == "")
      {
          $continente = "";
      }
      if (isset($continente) && !empty($continente))
      {
         $tmp_pos = (is_string($continente)) ? strpos($continente, "##@@") : false;
         if ($tmp_pos === false)
         { }
         else
         {
         $continente = substr($continente, 0, $tmp_pos);
         }
      }
?>
 <TR align="center">
  ...
  <TD width="100%" height="">
   ...
   <tr valign="top">
      ...
      <TD id='SC_region_label' class="scFilterLabelOdd"><?php echo (isset($this->New_label['region'])) ? $this->New_label['region'] : "" . $this->Ini->Nm_lang['lang_pais_fld_region'] . ""; ?></TD>
     <TD class="scFilterFieldOdd"> 
      <SELECT class="scFilterObjectOdd" id="SC_region_cond" name="region_cond" onChange="nm_campos_between(document.getElementById('id_vis_region'), this, 'region')">
       <OPTION value="qp" <?php if ("qp" == $region_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_like'] ?></OPTION>
       <OPTION value="np" <?php if ("np" == $region_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_not_like'] ?></OPTION>
       <OPTION value="eq" <?php if ("eq" == $region_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_exac'] ?></OPTION>
       <OPTION value="ep" <?php if ("ep" == $region_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_empty'] ?></OPTION>
      </SELECT>
       </TD>
     <TD  class="scFilterFieldOdd">
      <TABLE  border="0" cellpadding="0" cellspacing="0">
       <TR id="id_hide_region" <?php echo $str_hide_region?> valign="top">
 ...
 $nmgp_tab_label .= "region?#?" . $SC_Label . "?@?";
...
<?php
      ...
      {
          $sAutocompValue = $region;
      }
?>
<INPUT  type="text" id="SC_region" name="region" value="<?php echo NM_encode_input($region) ?>"  size=26 alt="{datatype: 'text', maxLength: 26, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}" style="display: none">
<input class="sc-js-input scFilterObjectOdd" type="text" id="id_ac_region" name="region_autocomp" size="26"  value="<?php echo NM_encode_input($sAutocompValue); ?>" alt="{datatype: 'text', maxLength: 26, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}">
      ...
      <TD id='SC_codigo_label' class="scFilterLabelEven"><?php echo (isset($this->New_label['codigo'])) ? $this->New_label['codigo'] : "" . $this->Ini->Nm_lang['lang_pais_fld_codigo'] . ""; ?> <span class="scFilterRequiredOdd">*</span></TD>
...
?>
<INPUT  type="text" id="SC_codigo" name="codigo" value="<?php echo NM_encode_input($codigo) ?>"  size=3 alt="{datatype: 'text', maxLength: 3, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}" style="display: none">
<input class="sc-js-input scFilterObjectEven" type="text" id="id_ac_codigo" name="codigo_autocomp" size="3"  value="<?php echo NM_encode_input($sAutocompValue); ?>" alt="{datatype: 'text', maxLength: 3, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}">
      ...
      <TD id='SC_nombre_label' class="scFilterLabelOdd"><?php echo (isset($this->New_label['nombre'])) ? $this->New_label['nombre'] : "" . $this->Ini->Nm_lang['lang_pais_fld_nombre'] . ""; ?></TD>
     <TD class="scFilterFieldOdd"> 
...
?>
<INPUT  type="text" id="SC_nombre" name="nombre" value="<?php echo NM_encode_input($nombre) ?>"  size=52 alt="{datatype: 'text', maxLength: 52, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}" style="display: none">
<input class="sc-js-input scFilterObjectOdd" type="text" id="id_ac_nombre" name="nombre_autocomp" size="52"  value="<?php echo NM_encode_input($sAutocompValue); ?>" alt="{datatype: 'text', maxLength: 52, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}">
      ...
      <TD id='SC_continente_label' class="scFilterLabelEven"><?php echo (isset($this->New_label['continente'])) ? $this->New_label['continente'] : "" . $this->Ini->Nm_lang['lang_pais_fld_continente'] . ""; ?></TD>
     <TD class="scFilterFieldEven"> 
      <SELECT class="scFilterObjectEven" id="SC_continente_cond" name="continente_cond" onChange="nm_campos_between(document.getElementById('id_vis_continente'), this, 'continente')">
       <OPTION value="qp" <?php if ("qp" == $continente_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_like'] ?></OPTION>
       <OPTION value="np" <?php if ("np" == $continente_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_not_like'] ?></OPTION>
       <OPTION value="eq" <?php if ("eq" == $continente_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_exac'] ?></OPTION>
       <OPTION value="ep" <?php if ("ep" == $continente_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_empty'] ?></OPTION>
      </SELECT>
       </TD>
     <TD  class="scFilterFieldEven">
      <TABLE  border="0" cellpadding="0" cellspacing="0">
       <TR id="id_hide_continente" <?php echo $str_hide_continente?> valign="top">
        <TD class="scFilterFieldFontEven">
           <?php
 $SC_Label = (isset($this->New_label['continente'])) ? $this->New_label['continente'] : "" . $this->Ini->Nm_lang['lang_pais_fld_continente'] . "";
 $nmgp_tab_label .= "continente?#?" . $SC_Label . "?@?";
 $date_sep_bw = " " . $this->Ini->Nm_lang['lang_srch_between_values'] . " ";
 if ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($date_sep_bw))
 {
     $date_sep_bw = sc_convert_encoding($date_sep_bw, $_SESSION['scriptcase']['charset'], "UTF-8");
...
<?php
      ...
      {
          $sAutocompValue = $continente;
      }
?>
<INPUT  type="text" id="SC_continente" name="continente" value="<?php echo NM_encode_input($continente) ?>"  size=10 alt="{datatype: 'text', maxLength: 10, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}" style="display: none">
<input class="sc-js-input scFilterObjectEven" type="text" id="id_ac_continente" name="continente_autocomp" size="10"  value="<?php echo NM_encode_input($sAutocompValue); ?>" alt="{datatype: 'text', maxLength: 10, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}">
...
?>
   <INPUT type="hidden" name="script_case_init" value="<?php echo NM_encode_input($this->Ini->sc_page); ?>"> 
<?php
   if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_pais']['orig_pesq']) && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pais']['orig_pesq'] == "grid")
   {
...
?>
 ...
 function limpa_form()
 {
   ...
   document.F1.region_cond.value = 'qp';
   nm_campos_between(document.getElementById('id_vis_region'), document.F1.region_cond, 'region');
   document.F1.region.value = "";
   document.F1.region_autocomp.value = "";
   ...
   document.F1.continente_cond.value = 'qp';
   nm_campos_between(document.getElementById('id_vis_continente'), document.F1.continente_cond, 'continente');
   document.F1.continente.value = "";
   document.F1.continente_autocomp.value = "";
 }
...
<?php
   ...
   function recupera_filtro($NM_filters, $Orig_Fil)
   {
      ...
      $tp_fields['SC_region_cond'] = 'cond';
      $tp_fields['SC_region'] = 'text_aut';
      $tp_fields['id_ac_region'] = 'text_aut';
      ...
      $tp_fields['SC_continente_cond'] = 'cond';
      $tp_fields['SC_continente'] = 'text_aut';
      $tp_fields['id_ac_continente'] = 'text_aut';
   ...
   {
      global $region_cond, $region, $region_autocomp,
             $codigo_cond, $codigo, $codigo_autocomp,
             $nombre_cond, $nombre, $nombre_autocomp,
             $continente_cond, $continente, $continente_autocomp, $nmgp_tab_label;
      ...
      if (!empty($region_autocomp) && empty($region))
      {
          $region = $region_autocomp;
      }
      ...
      if (!empty($continente_autocomp) && empty($continente))
      {
          $continente = $continente_autocomp;
      }
      $region_cond_salva = $region_cond; 
      if (!isset($region_input_2) || $region_input_2 == "")
      {
          $region_input_2 = $region;
      }
      ...
      $continente_cond_salva = $continente_cond; 
      if (!isset($continente_input_2) || $continente_input_2 == "")
      {
          $continente_input_2 = $continente;
      }
      if (!$this->NM_ajax_flag || $this->NM_ajax_opcao != "ajax_grid_search")
      {
          if ($codigo_cond != "nu" && $codigo_cond != "nn" && $codigo_cond != "ep" && $codigo_cond != "ne")
          {
              if ($codigo == "")
              {
                  if (!empty($this->Campos_Mens_erro)) {$this->Campos_Mens_erro .= "<br>";}$this->Campos_Mens_erro .= "" . $this->Ini->Nm_lang['lang_pais_fld_codigo'] . " : " . $this->Ini->Nm_lang['lang_errm_ajax_rqrd'];
              }
          }
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pais']['campos_busca']  = array(); 
      ...
      $Dyn_ok = false;
      $Grid_ok = false;
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pais']['campos_busca']['region'] = $region; 
      if (is_array($region) && !empty($region))
      {
          $Grid_ok = true;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pais']['Grid_search'][$I_Grid]['val'][0] = $region;
      }
      elseif ($region_cond_salva == "nu" || $region_cond_salva == "nn" || $region_cond_salva == "ep" || $region_cond_salva == "ne" || !empty($region))
      {
          $Grid_ok = true;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pais']['Grid_search'][$I_Grid]['val'][0][0] = $region;
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pais']['campos_busca']['region_cond'] = $region_cond_salva; 
      if ($Grid_ok)
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pais']['Grid_search'][$I_Grid]['cmp'] = "region"; 
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pais']['Grid_search'][$I_Grid]['opc'] = $region_cond_salva; 
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pais']['grid_pesq']['region'] = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pais']['Grid_search'][$I_Grid];
          $I_Grid++;
      }
      $Dyn_ok = false;
      $Grid_ok = false;
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pais']['campos_busca']['codigo'] = $codigo; 
      if (is_array($codigo) && !empty($codigo))
      {
          $Grid_ok = true;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pais']['Grid_search'][$I_Grid]['val'][0] = $codigo;
      }
      elseif ($codigo_cond_salva == "nu" || $codigo_cond_salva == "nn" || $codigo_cond_salva == "ep" || $codigo_cond_salva == "ne" || !empty($codigo))
      {
          $Grid_ok = true;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pais']['Grid_search'][$I_Grid]['val'][0][0] = $codigo;
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pais']['campos_busca']['codigo_cond'] = $codigo_cond_salva; 
      if ($Grid_ok)
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pais']['Grid_search'][$I_Grid]['cmp'] = "codigo"; 
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pais']['Grid_search'][$I_Grid]['opc'] = $codigo_cond_salva; 
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pais']['grid_pesq']['codigo'] = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pais']['Grid_search'][$I_Grid];
          $I_Grid++;
      }
      $Dyn_ok = false;
      $Grid_ok = false;
      ...
      $Dyn_ok = false;
      $Grid_ok = false;
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pais']['campos_busca']['continente'] = $continente; 
      if (is_array($continente) && !empty($continente))
      {
          $Grid_ok = true;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pais']['Grid_search'][$I_Grid]['val'][0] = $continente;
      }
      elseif ($continente_cond_salva == "nu" || $continente_cond_salva == "nn" || $continente_cond_salva == "ep" || $continente_cond_salva == "ne" || !empty($continente))
      {
          $Grid_ok = true;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pais']['Grid_search'][$I_Grid]['val'][0][0] = $continente;
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pais']['campos_busca']['continente_cond'] = $continente_cond_salva; 
      if ($Grid_ok)
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pais']['Grid_search'][$I_Grid]['cmp'] = "continente"; 
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pais']['Grid_search'][$I_Grid]['opc'] = $continente_cond_salva; 
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pais']['grid_pesq']['continente'] = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pais']['Grid_search'][$I_Grid];
          $I_Grid++;
      }
    ...
    if ($region != '') {
      ...
      if (!empty($nmgp_def_dados) && isset($cmp2) && !empty($cmp2))
      {
          ...
          $this->cmp_formatado['region'] = $cmp2;
      }
      elseif (!empty($nmgp_def_dados) && isset($cmp1) && !empty($cmp1))
      {
          ...
          $this->cmp_formatado['region'] = $cmp1;
      }
      else
      {
          $this->cmp_formatado['region'] = $region;
      }
      $nmgp_def_dados = array();
    if ($codigo != '') {
      $codigo_look = (is_string($codigo) ? substr($this->Db->qstr($codigo), 1, -1) : $codigo); 
    ...
    if ($continente != '') {
      ...
      if (!empty($nmgp_def_dados) && isset($cmp2) && !empty($cmp2))
      {
          ...
          $this->cmp_formatado['continente'] = $cmp2;
      }
      elseif (!empty($nmgp_def_dados) && isset($cmp1) && !empty($cmp1))
      {
          if ($_SESSION['scriptcase']['charset'] != "UTF-8")
          ...
          $this->cmp_formatado['continente'] = $cmp1;
      }
      else
      {
          $this->cmp_formatado['continente'] = $continente;
      }
   }
   ...
   function processa_campos($Res_comp='pesq')
   {
      ...
      if (isset($region) || $region_cond == "nu" || $region_cond == "nn" || $region_cond == "ep" || $region_cond == "ne")
      {
         $this->monta_condicao("region", $region_cond, $region, "", "region", "CHAR", false);
      }
      ...
      {
         $this->monta_condicao("codigo", $codigo_cond, $codigo, "", "codigo", "CHAR", false);
      }
      ...
      //----- $continente
      $this->Date_part = false;
      if (isset($continente) || $continente_cond == "nu" || $continente_cond == "nn" || $continente_cond == "ep" || $continente_cond == "ne")
      {
         $this->monta_condicao("continente", $continente_cond, $continente, "", "continente", "ENUM", false);
      }
   }
   ...
   function finaliza_resultado()
   {
      ...
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pais']['where_pesq_filtro'] = " (" . trim($this->comando_filtro) . ")";
      }
