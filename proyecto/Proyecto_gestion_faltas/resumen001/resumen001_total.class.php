...
class resumen001_total
{
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['resumen001']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['resumen001']['campos_busca']))
      { 
   ...
   //----- 
   function Calc_resumo_AREA_REGISTRO($destino_resumo, $res_export=false, $ind_compara="")
   {
      global $nm_lang;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['resumen001']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['resumen001']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('area_registro' => "AREA_REGISTRO");
      $cmps_quebra_atual = array("area_registro");
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['resumen001']['SC_force_fiels_gb']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['resumen001']['SC_force_fiels_gb']))
      {
          $cmps_quebra_atual = $_SESSION['sc_session'][$this->Ini->sc_page]['resumen001']['SC_force_fiels_gb'];
      }
      ...
      {
          if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['resumen001']['SC_clear_total']) && $_SESSION['sc_session'][$this->Ini->sc_page]['resumen001']['SC_clear_total']) {
      ...
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['resumen001']['SC_clear_total']);
      $ind_cmps  = 3;
      $ind_alias = "1";
      $cmp_dim   = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          ...
          $this->Res_Totaliza_AREA_REGISTRO($ind_qb, $cmp_gb, $arr_tots . $arr_compara, $group, $join, $cmps_gb, $cmps_gb1, $cmps_gb2, $cmps_gbS, $cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export);
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['resumen001']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['resumen001']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }

   function Res_Totaliza_AREA_REGISTRO($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export)
   {
      $sc_having = ((isset($parms_sub_sel['having']))) ? "  having " . $parms_sub_sel['having'] : "";
      $Tem_estat_manual = false;
      $where_ok = $this->sc_where_atual;
      $cmp_sql_tp_num = array('total_faltas' => 'N','porcentaje_faltas' => 'N');
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
         $cmd_simp = "select count(*), sum(TOTAL_FALTAS), sum(PORCENTAJE_FALTAS)#@#cmps_quebras#@# from (SELECT      fq.area_name AS AREA_REGISTRO,     COUNT(*) AS TOTAL_FALTAS,     (COUNT(*) * 100.0 / (SELECT COUNT(*) FROM faults_practitioners)) AS PORCENTAJE_FALTAS FROM faults_practitioners f INNER JOIN practitioners p1 ON f.practitioner_fault_id = p1.practitioner_id INNER JOIN practitioners p2 ON f.practitioner_register_id = p2.practitioner_id INNER JOIN practitioners p3 ON f.practitioner_execute_id = p3.practitioner_id JOIN practitioners AS pn ON pn.practitioner_id = f.practitioner_fault_id JOIN areas AS fq ON pn.area_id = fq.area_id GROUP BY fq.area_name) sc_sel_esp " . $where_ok;
         $comando  = "select count(*), sum(SC_metric1), sum(SC_metric2)#@#cmps_quebras#@# from (";
         $comando .= "select TOTAL_FALTAS as SC_metric1,PORCENTAJE_FALTAS as SC_metric2, " . $cmps_quebras1 . " from (SELECT      fq.area_name AS AREA_REGISTRO,     COUNT(*) AS TOTAL_FALTAS,     (COUNT(*) * 100.0 / (SELECT COUNT(*) FROM faults_practitioners)) AS PORCENTAJE_FALTAS FROM faults_practitioners f INNER JOIN practitioners p1 ON f.practitioner_fault_id = p1.practitioner_id INNER JOIN practitioners p2 ON f.practitioner_register_id = p2.practitioner_id INNER JOIN practitioners p3 ON f.practitioner_execute_id = p3.practitioner_id JOIN practitioners AS pn ON pn.practitioner_id = f.practitioner_fault_id JOIN areas AS fq ON pn.area_id = fq.area_id GROUP BY fq.area_name) sc_sel_esp1 " . $where_ok . ") SC_sel1 INNER JOIN (";
         $comando .= "select " . $cmps_quebras2 . " from (SELECT      fq.area_name AS AREA_REGISTRO,     COUNT(*) AS TOTAL_FALTAS,     (COUNT(*) * 100.0 / (SELECT COUNT(*) FROM faults_practitioners)) AS PORCENTAJE_FALTAS FROM faults_practitioners f INNER JOIN practitioners p1 ON f.practitioner_fault_id = p1.practitioner_id INNER JOIN practitioners p2 ON f.practitioner_register_id = p2.practitioner_id INNER JOIN practitioners p3 ON f.practitioner_execute_id = p3.practitioner_id JOIN practitioners AS pn ON pn.practitioner_id = f.practitioner_fault_id JOIN areas AS fq ON pn.area_id = fq.area_id GROUP BY fq.area_name) sc_sel_esp2 " . $where_ok . " group by " . $group . $sc_having . ") SC_sel2 ";
         $comando .= " ON " . $join;
      } 
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['resumen001']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['resumen001']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['resumen001']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['resumen001']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['area_registro']['reg'] = "S";
      $format_dimensions['area_registro']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              $SC_prep = $this->Ini->Get_format_dimension($Ind_sql, 'AREA_REGISTRO', $Cada_dim, $rt, $format_dimensions[$Cada_dim]['reg'], $format_dimensions[$Cada_dim]['msk']);
              $SC_orig = $Cada_dim . "_orig";
              $SC_graf = "val_grafico_" . $Cada_dim;
              $$Cada_dim = $SC_prep['fmt'];
              $$SC_orig = $SC_prep['orig'];
              if ($Cada_dim == "area_registro") {
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
          ...
          if ($Tem_estat_manual)
          {
              ...
              $vl_statistic = $this->Calc_statist_manual_AREA_REGISTRO($where_ok);
   ...
   //---- 
   function quebra_geral_AREA_REGISTRO($res_limit=false, $res_export=false)
   {
      global $nada, $nm_lang ;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['resumen001']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['resumen001']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*), sum(TOTAL_FALTAS), sum(PORCENTAJE_FALTAS) from (SELECT      fq.area_name AS AREA_REGISTRO,     COUNT(*) AS TOTAL_FALTAS,     (COUNT(*) * 100.0 / (SELECT COUNT(*) FROM faults_practitioners)) AS PORCENTAJE_FALTAS FROM faults_practitioners f INNER JOIN practitioners p1 ON f.practitioner_fault_id = p1.practitioner_id INNER JOIN practitioners p2 ON f.practitioner_register_id = p2.practitioner_id INNER JOIN practitioners p3 ON f.practitioner_execute_id = p3.practitioner_id JOIN practitioners AS pn ON pn.practitioner_id = f.practitioner_fault_id JOIN areas AS fq ON pn.area_id = fq.area_id GROUP BY fq.area_name) nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['resumen001']['where_pesq']; 
      } 
      else 
      { 
          $nm_comando = "select count(*), sum(TOTAL_FALTAS), sum(PORCENTAJE_FALTAS) from (SELECT      fq.area_name AS AREA_REGISTRO,     COUNT(*) AS TOTAL_FALTAS,     (COUNT(*) * 100.0 / (SELECT COUNT(*) FROM faults_practitioners)) AS PORCENTAJE_FALTAS FROM faults_practitioners f INNER JOIN practitioners p1 ON f.practitioner_fault_id = p1.practitioner_id INNER JOIN practitioners p2 ON f.practitioner_register_id = p2.practitioner_id INNER JOIN practitioners p3 ON f.practitioner_execute_id = p3.practitioner_id JOIN practitioners AS pn ON pn.practitioner_id = f.practitioner_fault_id JOIN areas AS fq ON pn.area_id = fq.area_id GROUP BY fq.area_name) nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['resumen001']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['resumen001']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['resumen001']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['resumen001']['tot_geral'][2] = $rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $rt->fields[2] = (string)$rt->fields[2]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['resumen001']['tot_geral'][3] = $rt->fields[2]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['resumen001']['contr_total_geral'] = "OK";
   } 

   //-----  area_registro
   function quebra_area_registro_AREA_REGISTRO($area_registro, $arg_sum_area_registro) 
   {
      global $tot_area_registro;
      $tot_area_registro = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*), sum(TOTAL_FALTAS), sum(PORCENTAJE_FALTAS) from (SELECT      fq.area_name AS AREA_REGISTRO,     COUNT(*) AS TOTAL_FALTAS,     (COUNT(*) * 100.0 / (SELECT COUNT(*) FROM faults_practitioners)) AS PORCENTAJE_FALTAS FROM faults_practitioners f INNER JOIN practitioners p1 ON f.practitioner_fault_id = p1.practitioner_id INNER JOIN practitioners p2 ON f.practitioner_register_id = p2.practitioner_id INNER JOIN practitioners p3 ON f.practitioner_execute_id = p3.practitioner_id JOIN practitioners AS pn ON pn.practitioner_id = f.practitioner_fault_id JOIN areas AS fq ON pn.area_id = fq.area_id GROUP BY fq.area_name) nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['resumen001']['where_pesq']; 
      } 
      ...
      { 
         $nm_comando .= " where AREA_REGISTRO" . $arg_sum_area_registro ; 
      } 
      else 
      { 
         $nm_comando .= " and AREA_REGISTRO" . $arg_sum_area_registro ; 
      } 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['resumen001']['opcao'] == "pdf" && isset($_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content']) && $_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content'] == 'Y') {
          $tot_area_registro[0] = NM_encode_input(sc_strip_script($area_registro));
      }
      else {
          $tot_area_registro[0] = sc_strip_script($area_registro) ; 
      }
      $tot_area_registro[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_area_registro[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_area_registro[3] = (string)$rt->fields[2]; 
      $rt->Close(); 
   } 
