...
class resumen003_total
{
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['resumen003']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['resumen003']['campos_busca']))
      { 
   ...
   function Calc_resumo_PERSONA($destino_resumo, $res_export=false, $ind_compara="")
   {
      global $nm_lang;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['resumen003']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['resumen003']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('persona' => "PERSONA");
      $cmps_quebra_atual = array("persona");
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['resumen003']['SC_force_fiels_gb']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['resumen003']['SC_force_fiels_gb']))
      {
          $cmps_quebra_atual = $_SESSION['sc_session'][$this->Ini->sc_page]['resumen003']['SC_force_fiels_gb'];
      }
      ...
      {
          if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['resumen003']['SC_clear_total']) && $_SESSION['sc_session'][$this->Ini->sc_page]['resumen003']['SC_clear_total']) {
      ...
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['resumen003']['SC_clear_total']);
      $ind_cmps  = 3;
      $ind_alias = "1";
      $cmp_dim   = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['resumen003']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['resumen003']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }
   ...
   function Res_Totaliza_PERSONA($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export)
   {
      ...
      $cmp_sql_tp_num = array('ingreso_de_faltas' => 'N','porcentaje' => 'N');
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
         $cmd_simp = "select count(*), sum(INGRESO_DE_FALTAS), sum(PORCENTAJE)#@#cmps_quebras#@# from (SELECT      g3.general_name AS PERSONA,     COUNT(*) AS INGRESO_DE_FALTAS,     (COUNT(*) * 100.0 / (SELECT COUNT(*) FROM faults_practitioners WHERE status_sanction_id IS NOT NULL)) AS PORCENTAJE FROM faults_practitioners f JOIN practitioners p3 ON f.practitioner_register_id = p3.practitioner_id JOIN general_data g3 ON p3.general_id = g3.general_id GROUP BY g3.general_name) sc_sel_esp " . $where_ok;
         $comando  = "select count(*), sum(SC_metric1), sum(SC_metric2)#@#cmps_quebras#@# from (";
         $comando .= "select INGRESO_DE_FALTAS as SC_metric1,PORCENTAJE as SC_metric2, " . $cmps_quebras1 . " from (SELECT      g3.general_name AS PERSONA,     COUNT(*) AS INGRESO_DE_FALTAS,     (COUNT(*) * 100.0 / (SELECT COUNT(*) FROM faults_practitioners WHERE status_sanction_id IS NOT NULL)) AS PORCENTAJE FROM faults_practitioners f JOIN practitioners p3 ON f.practitioner_register_id = p3.practitioner_id JOIN general_data g3 ON p3.general_id = g3.general_id GROUP BY g3.general_name) sc_sel_esp1 " . $where_ok . ") SC_sel1 INNER JOIN (";
         $comando .= "select " . $cmps_quebras2 . " from (SELECT      g3.general_name AS PERSONA,     COUNT(*) AS INGRESO_DE_FALTAS,     (COUNT(*) * 100.0 / (SELECT COUNT(*) FROM faults_practitioners WHERE status_sanction_id IS NOT NULL)) AS PORCENTAJE FROM faults_practitioners f JOIN practitioners p3 ON f.practitioner_register_id = p3.practitioner_id JOIN general_data g3 ON p3.general_id = g3.general_id GROUP BY g3.general_name) sc_sel_esp2 " . $where_ok . " group by " . $group . $sc_having . ") SC_sel2 ";
         $comando .= " ON " . $join;
      } 
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['resumen003']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['resumen003']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['resumen003']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['resumen003']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
   ...
   {
      global $nada, $nm_lang ;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['resumen003']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['resumen003']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*), sum(INGRESO_DE_FALTAS), sum(PORCENTAJE) from (SELECT      g3.general_name AS PERSONA,     COUNT(*) AS INGRESO_DE_FALTAS,     (COUNT(*) * 100.0 / (SELECT COUNT(*) FROM faults_practitioners WHERE status_sanction_id IS NOT NULL)) AS PORCENTAJE FROM faults_practitioners f JOIN practitioners p3 ON f.practitioner_register_id = p3.practitioner_id JOIN general_data g3 ON p3.general_id = g3.general_id GROUP BY g3.general_name) nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['resumen003']['where_pesq']; 
      } 
      else 
      { 
          $nm_comando = "select count(*), sum(INGRESO_DE_FALTAS), sum(PORCENTAJE) from (SELECT      g3.general_name AS PERSONA,     COUNT(*) AS INGRESO_DE_FALTAS,     (COUNT(*) * 100.0 / (SELECT COUNT(*) FROM faults_practitioners WHERE status_sanction_id IS NOT NULL)) AS PORCENTAJE FROM faults_practitioners f JOIN practitioners p3 ON f.practitioner_register_id = p3.practitioner_id JOIN general_data g3 ON p3.general_id = g3.general_id GROUP BY g3.general_name) nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['resumen003']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['resumen003']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['resumen003']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['resumen003']['tot_geral'][2] = $rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $rt->fields[2] = (string)$rt->fields[2]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['resumen003']['tot_geral'][3] = $rt->fields[2]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['resumen003']['contr_total_geral'] = "OK";
   } 
