...
class resumen004_total
{
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['resumen004']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['resumen004']['campos_busca']))
      { 
   ...
   //----- 
   function Calc_resumo_PERSONA($destino_resumo, $res_export=false, $ind_compara="")
   {
      global $nm_lang;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['resumen004']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['resumen004']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('persona' => "PERSONA");
      $cmps_quebra_atual = array("persona");
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['resumen004']['SC_force_fiels_gb']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['resumen004']['SC_force_fiels_gb']))
      {
          $cmps_quebra_atual = $_SESSION['sc_session'][$this->Ini->sc_page]['resumen004']['SC_force_fiels_gb'];
      }
      ...
      {
          if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['resumen004']['SC_clear_total']) && $_SESSION['sc_session'][$this->Ini->sc_page]['resumen004']['SC_clear_total']) {
      ...
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['resumen004']['SC_clear_total']);
      $ind_cmps  = 3;
      $ind_alias = "1";
      $cmp_dim   = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          ...
          $this->Res_Totaliza_PERSONA($ind_qb, $cmp_gb, $arr_tots . $arr_compara, $group, $join, $cmps_gb, $cmps_gb1, $cmps_gb2, $cmps_gbS, $cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export);
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['resumen004']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['resumen004']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }

   function Res_Totaliza_PERSONA($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export)
   {
      $sc_having = ((isset($parms_sub_sel['having']))) ? "  having " . $parms_sub_sel['having'] : "";
      $Tem_estat_manual = false;
      $where_ok = $this->sc_where_atual;
      $cmp_sql_tp_num = array('ejecucion_de_sanciones' => 'N','porcentaje' => 'N');
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
         $cmd_simp = "select count(*), sum(EJECUCION_DE_SANCIONES), sum(PORCENTAJE)#@#cmps_quebras#@# from (SELECT      g3.general_name AS PERSONA,     COUNT(*) AS EJECUCION_DE_SANCIONES,     (COUNT(*) * 100.0 / (SELECT COUNT(*) FROM faults_practitioners WHERE status_sanction_id IS NOT NULL)) AS PORCENTAJE FROM faults_practitioners f JOIN practitioners p3 ON f.practitioner_execute_id = p3.practitioner_id JOIN general_data g3 ON p3.general_id = g3.general_id GROUP BY g3.general_name) sc_sel_esp " . $where_ok;
         $comando  = "select count(*), sum(SC_metric1), sum(SC_metric2)#@#cmps_quebras#@# from (";
         $comando .= "select EJECUCION_DE_SANCIONES as SC_metric1,PORCENTAJE as SC_metric2, " . $cmps_quebras1 . " from (SELECT      g3.general_name AS PERSONA,     COUNT(*) AS EJECUCION_DE_SANCIONES,     (COUNT(*) * 100.0 / (SELECT COUNT(*) FROM faults_practitioners WHERE status_sanction_id IS NOT NULL)) AS PORCENTAJE FROM faults_practitioners f JOIN practitioners p3 ON f.practitioner_execute_id = p3.practitioner_id JOIN general_data g3 ON p3.general_id = g3.general_id GROUP BY g3.general_name) sc_sel_esp1 " . $where_ok . ") SC_sel1 INNER JOIN (";
         $comando .= "select " . $cmps_quebras2 . " from (SELECT      g3.general_name AS PERSONA,     COUNT(*) AS EJECUCION_DE_SANCIONES,     (COUNT(*) * 100.0 / (SELECT COUNT(*) FROM faults_practitioners WHERE status_sanction_id IS NOT NULL)) AS PORCENTAJE FROM faults_practitioners f JOIN practitioners p3 ON f.practitioner_execute_id = p3.practitioner_id JOIN general_data g3 ON p3.general_id = g3.general_id GROUP BY g3.general_name) sc_sel_esp2 " . $where_ok . " group by " . $group . $sc_having . ") SC_sel2 ";
         $comando .= " ON " . $join;
      } 
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['resumen004']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['resumen004']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['resumen004']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['resumen004']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['persona']['reg'] = "S";
      $format_dimensions['persona']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              $SC_prep = $this->Ini->Get_format_dimension($Ind_sql, 'PERSONA', $Cada_dim, $rt, $format_dimensions[$Cada_dim]['reg'], $format_dimensions[$Cada_dim]['msk']);
          ...
          if ($Tem_estat_manual)
          {
              ...
              $vl_statistic = $this->Calc_statist_manual_PERSONA($where_ok);
   ...
   //---- 
   function quebra_geral_PERSONA($res_limit=false, $res_export=false)
   {
      global $nada, $nm_lang ;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['resumen004']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['resumen004']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*), sum(EJECUCION_DE_SANCIONES), sum(PORCENTAJE) from (SELECT      g3.general_name AS PERSONA,     COUNT(*) AS EJECUCION_DE_SANCIONES,     (COUNT(*) * 100.0 / (SELECT COUNT(*) FROM faults_practitioners WHERE status_sanction_id IS NOT NULL)) AS PORCENTAJE FROM faults_practitioners f JOIN practitioners p3 ON f.practitioner_execute_id = p3.practitioner_id JOIN general_data g3 ON p3.general_id = g3.general_id GROUP BY g3.general_name) nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['resumen004']['where_pesq']; 
      } 
      else 
      { 
          $nm_comando = "select count(*), sum(EJECUCION_DE_SANCIONES), sum(PORCENTAJE) from (SELECT      g3.general_name AS PERSONA,     COUNT(*) AS EJECUCION_DE_SANCIONES,     (COUNT(*) * 100.0 / (SELECT COUNT(*) FROM faults_practitioners WHERE status_sanction_id IS NOT NULL)) AS PORCENTAJE FROM faults_practitioners f JOIN practitioners p3 ON f.practitioner_execute_id = p3.practitioner_id JOIN general_data g3 ON p3.general_id = g3.general_id GROUP BY g3.general_name) nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['resumen004']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['resumen004']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['resumen004']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['resumen004']['tot_geral'][2] = $rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $rt->fields[2] = (string)$rt->fields[2]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['resumen004']['tot_geral'][3] = $rt->fields[2]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['resumen004']['contr_total_geral'] = "OK";
   } 

   //-----  persona
   function quebra_persona_PERSONA($persona, $arg_sum_persona) 
   {
      global $tot_persona;
      $tot_persona = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      ...
      { 
         $nm_comando .= " where PERSONA" . $arg_sum_persona ; 
      } 
      else 
      { 
         $nm_comando .= " and PERSONA" . $arg_sum_persona ; 
      } 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['resumen004']['opcao'] == "pdf" && isset($_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content']) && $_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content'] == 'Y') {
          $tot_persona[0] = NM_encode_input(sc_strip_script($persona));
      }
      else {
          $tot_persona[0] = sc_strip_script($persona) ; 
      }
      $tot_persona[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_persona[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_persona[3] = (string)$rt->fields[2]; 
      $rt->Close(); 
   } 
