...
function scEventControl_init(iSeqRow) {
  scEventControl_data["id_stage_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
  scEventControl_data["stage_info_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
}

function scEventControl_active(iSeqRow) {
  if (scEventControl_data["id_stage_" + iSeqRow] && scEventControl_data["id_stage_" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["id_stage_" + iSeqRow] && scEventControl_data["id_stage_" + iSeqRow]["change"]) {
    return true;
  }
  if (scEventControl_data["stage_info_" + iSeqRow] && scEventControl_data["stage_info_" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["stage_info_" + iSeqRow] && scEventControl_data["stage_info_" + iSeqRow]["change"]) {
    return true;
  }
  return false;
...
function scJQEventsAdd(iSeqRow) {
  $('#id_sc_field_id_stage_' + iSeqRow).bind('blur', function() { sc_form_tb_stages_id_stage__onblur(this, iSeqRow) })
                                       .bind('change', function() { sc_form_tb_stages_id_stage__onchange(this, iSeqRow) })
                                       .bind('focus', function() { sc_form_tb_stages_id_stage__onfocus(this, iSeqRow) });
  $('#id_sc_field_stage_info_' + iSeqRow).bind('blur', function() { sc_form_tb_stages_stage_info__onblur(this, iSeqRow) })
                                         .bind('change', function() { sc_form_tb_stages_stage_info__onchange(this, iSeqRow) })
                                         .bind('focus', function() { sc_form_tb_stages_stage_info__onfocus(this, iSeqRow) });
} // scJQEventsAdd

function sc_form_tb_stages_id_stage__onblur(oThis, iSeqRow) {
  do_ajax_form_tb_stages_validate_id_stage_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_tb_stages_id_stage__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_tb_stages_id_stage__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}

function sc_form_tb_stages_stage_info__onblur(oThis, iSeqRow) {
  do_ajax_form_tb_stages_validate_stage_info_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_tb_stages_stage_info__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_tb_stages_stage_info__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}
...
function displayChange_block_0(status) {
	displayChange_field("id_stage_", "", status);
	displayChange_field("stage_info_", "", status);
}

function displayChange_row(row, status) {
	displayChange_field_id_stage_(row, status);
	displayChange_field_stage_info_(row, status);
}
...
function displayChange_field(field, row, status) {
	...
	if ("stage_info_" == field) {
		displayChange_field_stage_info_(row, status);
	}
}

function displayChange_field_id_stage_(row, status) {
    var fieldId;
}

function displayChange_field_stage_info_(row, status) {
    var fieldId;
}
...
function scHidePage(pageNo) {
	$("#id_form_tb_stages_form" + pageNo).hide();
}
            ...
            if(!$(t).is('img')){
                ...
                switch(field){
                     ...
                     if(usa_read_only && $('a',$('#id_read_on_'+field+iSeqRow)).length == 0){
                         $(t).html("<a href=\"javascript:nm_mostra_doc('0', '"+rs2+"', 'form_tb_stages')\">"+$('#id_read_on_'+field+iSeqRow).text()+"</a>");
                     }
                }
