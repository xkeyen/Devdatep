...
class grid_tickets_unassigned_xls
{
   var $Db;
   var $Erro;
   var $Ini;
...
function actionBar_getStateHide($buttonName)
{
   ...
   function inicializa_vars()
   {
      ...
      { 
          ...
          if (!is_file($this->Ini->root . $this->Ini->path_link . "grid_tickets_unassigned/grid_tickets_unassigned_res_xls.class.php"))
          {
              $this->Tem_xls_res  = false;
          }
          if ($this->Tem_xls_res)
          { 
              require_once($this->Ini->path_aplicacao . "grid_tickets_unassigned_res_xls.class.php");
              $this->Res_xls = new grid_tickets_unassigned_res_xls();
              $this->prep_modulos("Res_xls");
          } 
          $this->Arquivo    = "sc_xls";
          $this->Arquivo   .= "_" . date("YmdHis") . "_" . rand(0, 1000);
          $this->Arq_zip    = $this->Arquivo . "_grid_tickets_unassigned.zip";
          $this->Arquivo   .= "_grid_tickets_unassigned" . $this->Xls_tp;
          $this->Tit_doc    = "grid_tickets_unassigned" . $this->Xls_tp;
      ...
      if (!$_SESSION['sc_session'][$this->Ini->sc_page]['grid_tickets_unassigned']['embutida'] && !$this->Ini->sc_export_ajax) {
          ...
          $this->pb->setReturnOption($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tickets_unassigned']['xls_return']);
          if ($this->Tem_xls_res) {
   ...
   function grava_arquivo()
   {
      ...
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tickets_unassigned']['field_order'] as $Cada_cmp)
      {
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tickets_unassigned']['xls_name']))
      {
          $Pos = strrpos($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tickets_unassigned']['xls_name'], ".");
          if ($Pos === false) {
              $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tickets_unassigned']['xls_name'] .= $this->Xls_tp;
          }
          $this->Arquivo = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tickets_unassigned']['xls_name'];
          $this->Arq_zip = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tickets_unassigned']['xls_name'];
          $this->Tit_doc = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tickets_unassigned']['xls_name'];
          $Pos = strrpos($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tickets_unassigned']['xls_name'], ".");
          if ($Pos !== false) {
              $this->Arq_zip = substr($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tickets_unassigned']['xls_name'], 0, $Pos);
          }
          $this->Arq_zip .= ".zip";
          $this->Tit_zip  = $this->Arq_zip;
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tickets_unassigned']['xls_name']);
          $this->Xls_f = $this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo;
          $this->Zip_f = $this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arq_zip;
      }
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tickets_unassigned']['embutida_label']) && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tickets_unassigned']['embutida_label'])
      { 
          $this->count_span = 0;
          $this->Xls_row++;
          $this->proc_label();
...
$_SESSION['scriptcase']['grid_tickets_unassigned']['contr_erro'] = 'off'; 
         ...
         if (isset($this->NM_Row_din) && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_tickets_unassigned']['embutida'])
         { 
      ...
      $this->xls_set_style();
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tickets_unassigned']['embutida'] && $prim_reg)
      { 
          $this->proc_label();
          $this->xls_sub_cons_copy_label($this->Xls_row);
      ...
      if (isset($this->NM_Col_din) && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_tickets_unassigned']['embutida'])
      { 
      ...
      { 
          if ($this->Tem_xls_res)
          { 
              $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tickets_unassigned']['xls_res_grid'] = true;
              if (!$this->Ini->sc_export_ajax) {
              ...
              if ($this->Use_phpspreadsheet) {
                  $Xls_res = \PhpOffice\PhpSpreadsheet\IOFactory::load($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tickets_unassigned']['xls_res_sheet']);
              }
              else {
                  $Xls_res = PHPExcel_IOFactory::load($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tickets_unassigned']['xls_res_sheet']);
              }
              foreach($Xls_res->getAllSheets() as $sheet)
              {
                  $this->Xls_dados->addExternalSheet($sheet);
              }
              unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tickets_unassigned']['xls_res_grid']);
              unlink($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tickets_unassigned']['xls_res_sheet']);
          } 
          if (!$this->Ini->sc_export_ajax) {
   ...
   function NM_export_ticket_ticketdate()
   {
         ...
         $this->NM_ctrl_style[$current_cell_ref]['end'] = $this->Xls_row;
      if (!empty($this->ticket_ticketdate))
      {
         ...
         if ($this->Use_phpspreadsheet) {
             $this->Nm_ActiveSheet->setCellValueExplicit($current_cell_ref . $this->Xls_row, $this->ticket_ticketdate, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
         }
         else {
             $this->Nm_ActiveSheet->setCellValueExplicit($current_cell_ref . $this->Xls_row, $this->ticket_ticketdate, PHPExcel_Cell_DataType::TYPE_STRING);
         }
         $this->Xls_col++;
   }
   //----- ticket_customerid
   function NM_export_ticket_customerid()
   {
         ...
         if (is_numeric($this->look_ticket_customerid))
         {
             $this->NM_ctrl_style[$current_cell_ref]['format'] = '#,##0';
         }
         $this->Nm_ActiveSheet->setCellValue($current_cell_ref . $this->Xls_row, $this->look_ticket_customerid);
         $this->Xls_col++;
   }
   //----- ticket_categoryid
   ...
   function NM_export_ticketpriority_ticketpriorityid()
   {
         ...
         if (is_numeric($this->look_ticketpriority_ticketpriorityid))
         {
             $this->NM_ctrl_style[$current_cell_ref]['format'] = '#,##0';
         }
         $this->Nm_ActiveSheet->setCellValue($current_cell_ref . $this->Xls_row, $this->look_ticketpriority_ticketpriorityid);
         $this->Xls_col++;
   }
   //----- ticket_assign
   function NM_export_ticket_assign()
   {
         ...
         if ($this->Use_phpspreadsheet) {
             $this->Nm_ActiveSheet->setCellValueExplicit($current_cell_ref . $this->Xls_row, $this->ticket_assign, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
         }
         else {
             $this->Nm_ActiveSheet->setCellValueExplicit($current_cell_ref . $this->Xls_row, $this->ticket_assign, PHPExcel_Cell_DataType::TYPE_STRING);
         }
         $this->Xls_col++;
   }
   //----- ticket_tickettrack
   ...
   //----- ticket_ticketdate
   function NM_sub_cons_ticket_ticketdate()
   {
      if (!empty($this->ticket_ticketdate))
      {
         $conteudo_x =  $this->ticket_ticketdate;
         ...
         $this->ticket_ticketdate = NM_charset_to_utf8($this->ticket_ticketdate);
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['data']   = $this->ticket_ticketdate;
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['align']  = "left";
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['type']   = "char";
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['format'] = "";
         $this->Xls_col++;
   ...
   //----- ticket_customerid
   function NM_sub_cons_ticket_customerid()
   {
         $this->look_ticket_customerid = NM_charset_to_utf8($this->look_ticket_customerid);
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['data']   = $this->look_ticket_customerid;
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['align']  = "left";
   ...
   //----- ticketpriority_ticketpriorityid
   function NM_sub_cons_ticketpriority_ticketpriorityid()
   {
         $this->look_ticketpriority_ticketpriorityid = NM_charset_to_utf8($this->look_ticketpriority_ticketpriorityid);
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['data']   = $this->look_ticketpriority_ticketpriorityid;
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['align']  = "left";
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['type']   = "num";
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['format'] = "#,##0";
         $this->Xls_col++;
   ...
   //----- ticket_assign
   function NM_sub_cons_ticket_assign()
   {
         $this->ticket_assign = html_entity_decode($this->ticket_assign, ENT_COMPAT, $_SESSION['scriptcase']['charset']);
         $this->ticket_assign = strip_tags($this->ticket_assign);
         $this->ticket_assign = NM_charset_to_utf8($this->ticket_assign);
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['data']   = $this->ticket_assign;
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['align']  = "left";
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['type']   = "char";
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['format'] = "";
         $this->Xls_col++;
   ...
   {
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tickets_unassigned']['nolabel']) || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tickets_unassigned']['nolabel'])
       {
   ...
   {
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tickets_unassigned']['xls_file']);
      if (is_file($this->Xls_f))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tickets_unassigned']['xls_file'] = $this->Xls_f;
      }
...
<HEAD>
 <TITLE><?php echo $this->Ini->Nm_lang['lang_staff_ticketsunassigned'] ?> :: Excel</TITLE>
...
<INPUT type="hidden" name="script_case_init" value="<?php echo NM_encode_input($this->Ini->sc_page); ?>"> 
<INPUT type="hidden" name="nmgp_opcao" value="<?php echo NM_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tickets_unassigned']['xls_return']); ?>"> 
</FORM> 
</BODY>
</HTML>
<?php
