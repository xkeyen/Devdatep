...
class grid_operator_ticketsdetail_xls
{
   var $Db;
   var $Erro;
   var $Ini;
...
function actionBar_isValidState($buttonName, $buttonState)
{
   ...
   function inicializa_vars()
   {
      ...
      { 
          ...
          if (!is_file($this->Ini->root . $this->Ini->path_link . "grid_operator_ticketsdetail/grid_operator_ticketsdetail_res_xls.class.php"))
          {
              $this->Tem_xls_res  = false;
          }
          if ($this->Tem_xls_res)
          { 
              require_once($this->Ini->path_aplicacao . "grid_operator_ticketsdetail_res_xls.class.php");
              $this->Res_xls = new grid_operator_ticketsdetail_res_xls();
              $this->prep_modulos("Res_xls");
          } 
          $this->Arquivo    = "sc_xls";
          $this->Arquivo   .= "_" . date("YmdHis") . "_" . rand(0, 1000);
          $this->Arq_zip    = $this->Arquivo . "_grid_operator_ticketsdetail.zip";
          $this->Arquivo   .= "_grid_operator_ticketsdetail" . $this->Xls_tp;
   ...
   function grava_arquivo()
   {
      ...
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_operator_ticketsdetail']['field_order'] as $Cada_cmp)
      {
...
$_SESSION['scriptcase']['grid_operator_ticketsdetail']['contr_erro'] = 'off'; 
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_operator_ticketsdetail']['embutida_label']) && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_operator_ticketsdetail']['embutida_label'])
      { 
          $this->count_span = 0;
          $this->Xls_row++;
          $this->proc_label();
     ...
     $nm_houve_quebra = "N";
         ...
         if (isset($this->NM_Row_din) && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_operator_ticketsdetail']['embutida'])
         { 
      ...
      $this->xls_set_style();
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_operator_ticketsdetail']['embutida'] && $prim_reg)
      { 
          $this->proc_label();
          $this->xls_sub_cons_copy_label($this->Xls_row);
      ...
      if (isset($this->NM_Col_din) && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_operator_ticketsdetail']['embutida'])
      { 
      ...
      { 
          if ($this->Tem_xls_res)
          { 
              $_SESSION['sc_session'][$this->Ini->sc_page]['grid_operator_ticketsdetail']['xls_res_grid'] = true;
              if (!$this->Ini->sc_export_ajax) {
              ...
              if ($this->Use_phpspreadsheet) {
                  $Xls_res = \PhpOffice\PhpSpreadsheet\IOFactory::load($_SESSION['sc_session'][$this->Ini->sc_page]['grid_operator_ticketsdetail']['xls_res_sheet']);
              }
              else {
                  $Xls_res = PHPExcel_IOFactory::load($_SESSION['sc_session'][$this->Ini->sc_page]['grid_operator_ticketsdetail']['xls_res_sheet']);
              }
              foreach($Xls_res->getAllSheets() as $sheet)
              {
                  $this->Xls_dados->addExternalSheet($sheet);
              }
              unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_operator_ticketsdetail']['xls_res_grid']);
              unlink($_SESSION['sc_session'][$this->Ini->sc_page]['grid_operator_ticketsdetail']['xls_res_sheet']);
          } 
          if (!$this->Ini->sc_export_ajax) {
   ...
   { 
      ...
      { 
          ...
          if ($Cada_col == "message" && (!isset($this->NM_cmp_hidden[$Cada_col]) || $this->NM_cmp_hidden[$Cada_col] != "off"))
          {
              $this->arr_span['message'] = $this->count_span;
              $this->Emb_label_cols_message = 0;
              $_SESSION['sc_session'][$this->Ini->sc_page]['grid_operator_ticketsmessage']['embutida'] = true;
              $_SESSION['sc_session'][$this->Ini->sc_page]['grid_operator_ticketsmessage']['embutida_label'] = true;
                  ...
                  if (isset($_SESSION['scriptcase']['export_return']))
                  {
                     foreach ($_SESSION['scriptcase']['export_return']['label'] as $col => $dados)
                     {
                         if (isset($dados['col_span_i'])) {
                             $this->Emb_label_cols_message += $dados['col_span_i'];
                         }
                         elseif (isset($dados['col_span_f'])) {
                             $this->Emb_label_cols_message += $dados['col_span_f'];
                         }
                         else {
                             $this->Emb_label_cols_message++;
                         }
                     }
                  }
                  $this->count_span += $this->Emb_label_cols_message;
              }
              ...
              if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_operator_ticketsdetail']['embutida'])
              { 
                  $this->arr_export['label'][$this->Xls_col]['col_span_f'] = $this->Emb_label_cols_message;
                  $this->Xls_col++;
              }
              else
              { 
                  $this->Xls_col += $this->Emb_label_cols_message;
              } 
          }
      } 
      $this->Xls_col = 0;
   ...
   //----- message
   function NM_sub_cons_message()
   {
         $this->Rows_sub_message = array();
         $GLOBALS["script_case_init"] = $this->Ini->sc_page;
         $GLOBALS["nmgp_parms"] = "nmgp_opcao?#?xls?@?v_ticketid?#?" . str_replace("'", "@aspass@", $this->Orig_ticket_ticketid) . "?@?";
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_operator_ticketsmessage']['embutida'] = true;
         if (method_exists($this->grid_operator_ticketsmessage, "controle"))
         {
             $this->grid_operator_ticketsmessage->controle();
             ...
             if (isset($_SESSION['scriptcase']['export_return']))
             {
                 ...
                 if (isset($_SESSION['scriptcase']['export_return']['lines']))
                 {
                     ...
                     foreach ($_SESSION['scriptcase']['export_return']['lines'] as $line => $cols)
                     {
                         ...
                         foreach ($cols as $icol => $dados)
                         {
                             $this->Rows_sub_message[$this->row_sub][$icol] = $dados;
                             if ($prim_col && $this->row_sub > 1)
                             {
                                 if (isset($this->Rows_sub_message[$this->row_sub][$icol]['col_span_i']))
                                 {
                                    $this->Rows_sub_message[$this->row_sub][$icol]['col_span_i'] += $this->arr_span['message'];
                                 }
                                 else
                                 {
                                    $this->Rows_sub_message[$this->row_sub][$icol]['col_span_i'] = $this->arr_span['message'];
   ...
   {
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_operator_ticketsdetail']['nolabel']) || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_operator_ticketsdetail']['nolabel'])
       {
