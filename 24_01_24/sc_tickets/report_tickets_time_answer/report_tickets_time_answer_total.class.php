...
class report_tickets_time_answer_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['report_tickets_time_answer']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['report_tickets_time_answer']['campos_busca']))
      { 
   ...
   //----- 
   function Calc_resumo_q1($destino_resumo, $res_export=false, $ind_compara="")
   {
      global $nm_lang;
      $this->nm_data = new nm_data("en_us");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['report_tickets_time_answer']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['report_tickets_time_answer']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('timeelapsedanswer' => "timeelapsedanswer");
      $cmps_quebra_atual = array("timeelapsedanswer");
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['report_tickets_time_answer']['SC_force_fiels_gb']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['report_tickets_time_answer']['SC_force_fiels_gb']))
      {
          $cmps_quebra_atual = $_SESSION['sc_session'][$this->Ini->sc_page]['report_tickets_time_answer']['SC_force_fiels_gb'];
      }
      ...
      {
          if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['report_tickets_time_answer']['SC_clear_total']) && $_SESSION['sc_session'][$this->Ini->sc_page]['report_tickets_time_answer']['SC_clear_total']) {
      ...
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['report_tickets_time_answer']['SC_clear_total']);
      $ind_cmps  = 1;
      $ind_alias = "1";
      $cmp_dim   = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          ...
          $this->Res_Totaliza_q1($ind_qb, $cmp_gb, $arr_tots . $arr_compara, $group, $join, $cmps_gb, $cmps_gb1, $cmps_gb2, $cmps_gbS, $cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export);
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['report_tickets_time_answer']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['report_tickets_time_answer']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }

   function Res_Totaliza_q1($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export)
   {
      $sc_having = ((isset($parms_sub_sel['having']))) ? "  having " . $parms_sub_sel['having'] : "";
      $Tem_estat_manual = false;
      $where_ok = $this->sc_where_atual;
      $cmp_sql_tp_num = array('ticketid' => 'N','ownerid' => 'N','categoryid' => 'N');
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['report_tickets_time_answer']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['report_tickets_time_answer']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['report_tickets_time_answer']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['report_tickets_time_answer']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['timeelapsedanswer']['reg'] = "S";
      $format_dimensions['timeelapsedanswer']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              $SC_prep = $this->Ini->Get_format_dimension($Ind_sql, 'q1', $Cada_dim, $rt, $format_dimensions[$Cada_dim]['reg'], $format_dimensions[$Cada_dim]['msk']);
              $SC_orig = $Cada_dim . "_orig";
              $SC_graf = "val_grafico_" . $Cada_dim;
              $$Cada_dim = $SC_prep['fmt'];
              $$SC_orig = $SC_prep['orig'];
              if ($Cada_dim == "timeelapsedanswer") {
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
          ...
          if ($Tem_estat_manual)
          {
              ...
              $vl_statistic = $this->Calc_statist_manual_q1($where_ok);
   ...
   //---- 
   function quebra_geral_q1($res_limit=false, $res_export=false)
   {
      global $nada, $nm_lang ;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['report_tickets_time_answer']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['report_tickets_time_answer']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['report_tickets_time_answer']['where_pesq']; 
      } 
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['report_tickets_time_answer']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['report_tickets_time_answer']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['report_tickets_time_answer']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['report_tickets_time_answer']['contr_total_geral'] = "OK";
   } 

   //-----  timeelapsedanswer
   function quebra_timeelapsedanswer_q1($timeelapsedanswer, $arg_sum_timeelapsedanswer) 
   {
      global $tot_timeelapsedanswer;
      $tot_timeelapsedanswer = array() ;  
      $tot_timeelapsedanswer[0] = $timeelapsedanswer ; 
   }
