...
class report_tickets_month_categories_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['report_tickets_month_categories']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['report_tickets_month_categories']['campos_busca']))
      { 
   ...
   //----- 
   function Calc_resumo_groupby_1($destino_resumo, $res_export=false, $ind_compara="")
   {
      global $nm_lang, $fld_ico_detail;
      $this->nm_data = new nm_data("en_us");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['report_tickets_month_categories']['sql_tot_res']);
      ...
      { 
          ...
          $this->ticketdate_2 = (isset($Busca_temp['ticketdate_input_2'])) ? $Busca_temp['ticketdate_input_2'] : ""; 
          $this->categoryid = (isset($Busca_temp['categoryid'])) ? $Busca_temp['categoryid'] : ""; 
          $tmp_pos = (is_string($this->categoryid)) ? strpos($this->categoryid, "##@@") : false;
          if ($tmp_pos !== false && !is_array($this->categoryid))
          {
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['report_tickets_month_categories']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('ticketdate' => "ticketdate");
      $cmps_quebra_atual = array("ticketdate");
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['report_tickets_month_categories']['SC_force_fiels_gb']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['report_tickets_month_categories']['SC_force_fiels_gb']))
      {
          $cmps_quebra_atual = $_SESSION['sc_session'][$this->Ini->sc_page]['report_tickets_month_categories']['SC_force_fiels_gb'];
      }
      ...
      {
          if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['report_tickets_month_categories']['SC_clear_total']) && $_SESSION['sc_session'][$this->Ini->sc_page]['report_tickets_month_categories']['SC_clear_total']) {
      ...
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['report_tickets_month_categories']['SC_clear_total']);
      $ind_cmps  = 1;
      $ind_alias = "1";
      $cmp_dim   = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          ...
          $this->Res_Totaliza_groupby_1($ind_qb, $cmp_gb, $arr_tots . $arr_compara, $group, $join, $cmps_gb, $cmps_gb1, $cmps_gb2, $cmps_gbS, $cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export);
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['report_tickets_month_categories']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['report_tickets_month_categories']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }

   function Res_Totaliza_groupby_1($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export)
   {
      $sc_having = ((isset($parms_sub_sel['having']))) ? "  having " . $parms_sub_sel['having'] : "";
      $Tem_estat_manual = false;
      $where_ok = $this->sc_where_atual;
      $cmp_sql_tp_num = array('ticketid' => 'N','ticketpriorityid' => 'N','ownerid' => 'N','customerid' => 'N','categoryid' => 'N','customerrating' => 'N');
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['report_tickets_month_categories']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['report_tickets_month_categories']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['report_tickets_month_categories']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['report_tickets_month_categories']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['ticketdate']['reg'] = "N";
      $format_dimensions['ticketdate']['msk'] = "F/Y";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              $SC_prep = $this->Ini->Get_format_dimension($Ind_sql, 'groupby_1', $Cada_dim, $rt, $format_dimensions[$Cada_dim]['reg'], $format_dimensions[$Cada_dim]['msk']);
          ...
          if ($Tem_estat_manual)
          {
              ...
              $vl_statistic = $this->Calc_statist_manual_groupby_1($where_ok);
   ...
   //---- 
   function quebra_geral_groupby_1($res_limit=false, $res_export=false)
   {
      global $nada, $nm_lang , $categoryid;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['report_tickets_month_categories']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['report_tickets_month_categories']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['report_tickets_month_categories']['where_pesq']; 
      } 
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['report_tickets_month_categories']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['report_tickets_month_categories']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['report_tickets_month_categories']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['report_tickets_month_categories']['contr_total_geral'] = "OK";
   } 

   //-----  ticketdate
   function quebra_ticketdate_groupby_1($ticketdate, $arg_sum_ticketdate) 
   {
      global $tot_ticketdate, $categoryid;
      $tot_ticketdate = array() ;  
      $tot_ticketdate[0] = $ticketdate ; 
   }
