...
$old_error_handler = set_error_handler("form_languages_scriptcase_error_handler");

function form_languages_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="", $err_usr=false)
{
    ...
    if (isset($_SESSION['scriptcase']['form_languages']['contr_erro']) && $_SESSION['scriptcase']['form_languages']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_form_languages->contr_form_languages->Apl_com_erro = true;
            if ($inicial_form_languages->contr_form_languages->NM_ajax_flag)
            {
                $inicial_form_languages->contr_form_languages->NM_ajax_info['errList']['geral_form_languages'][] = $msg;
                return;
            }
            form_languages_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function form_languages_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['form_languages'])  && '' != $_SESSION['scriptcase']['error_icon']['form_languages'])  ? $_SESSION['scriptcase']['error_icon']['form_languages']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['form_languages']) && '' != $_SESSION['scriptcase']['error_close']['form_languages']) ? $_SESSION['scriptcase']['error_close']['form_languages'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['form_languages']) && '' != $_SESSION['scriptcase']['error_span_title']['form_languages']) ? $_SESSION['scriptcase']['error_span_title']['form_languages'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['form_languages']) && '' != $_SESSION['scriptcase']['error_icon_title']['form_languages']) ? $_SESSION['scriptcase']['error_icon_title']['form_languages'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['form_languages']['error_buffer'] .= ob_get_flush();
}
