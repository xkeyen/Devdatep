...
class grid_operator_ticketsmessage_res_xml
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function monta_xml()
   {
      ...
      $this->grava_arquivo();
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_operator_ticketsmessage']['xml_res_grid']))
      {
          return;
      }
   ...
   function inicializa_vars()
   {
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_operator_ticketsmessage']['xml_res_grid']) && !$this->Ini->sc_export_ajax) {
   ...
   function grava_arquivo()
   {
      ...
      if ($this->New_Format)
      {
          fwrite($xml_f, "<grid_operator_ticketsmessage>\r\n");
      }
      if ($this->Grava_view)
      {
          ...
          if ($this->New_Format)
          {
              fwrite($xml_v, "<grid_operator_ticketsmessage>\r\n");
          }
      }
      ...
      $this->array_titulos = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_operator_ticketsmessage']['arr_export']['label'];
      $this->array_linhas  = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_operator_ticketsmessage']['arr_export']['data'];
      $contr_rowspan = array();
      $tit_rowspan   = array();
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_operator_ticketsmessage']['xml_res_grid']))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_operator_ticketsmessage']['xml_res_file']['xml'] = $this->Xml_f;
          if ($this->Grava_view)
          {
              $_SESSION['sc_session'][$this->Ini->sc_page]['grid_operator_ticketsmessage']['xml_res_file']['view'] = $this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo_view;
          }
      }
   ...
   function grava_linha($xml_f)
   {
      ...
      foreach ($this->array_linhas as $lines)
      {
          ...
          if (!$this->New_Format)
          {
              $xml_registro = "<grid_operator_ticketsmessage";
          }
          foreach ($lines as $columns)
          {
              if (0 <= $columns['level'])
	...
	$str_sql = "SELECT content,title FROM ticket_email_tpl M 
      ...
      { 
          ...
          { 
                 for ($SCx = 0; $SCx < $nm_count; $SCx++)
                 { 
                        $dataset1[$SCy] [$SCx] = $SCrx->fields[$SCx];
                 }
                 $SCy++; 
                 $SCrx->MoveNext();
          } 
      ...
      { 
          $dataset1 = false;
          $dataset1_erro = $this->Db->ErrorMsg();
      } 
